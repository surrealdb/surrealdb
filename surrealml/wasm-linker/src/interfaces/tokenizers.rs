...
///
/// # Arguments
/// * `model` – Pointer to a **non-null**, NUL-terminated UTF-8 C string specifying the model
///   identifier (e.g. `b"mistralai/Mistral-7B-v0.1\0"`).
/// * `hf_token` – Pointer to a NUL-terminated UTF-8 C string holding a Hugging Face access token
///   **or `NULL`**.   If `hf_token` is `NULL` the tokenizer is loaded from local resources;
///   otherwise it is fetched over HTTP with the supplied token.
///
/// # Returns
/// A [`TokenizerReturn`]:
/// * **Success** – `is_error == 0` and `tokenizer_handle` contains a pointer to a heap-allocated
///   `TokenizerHandle`.   The caller becomes responsible for releasing that handle via the
///   `free_tokenizer` (or similarly named) function exported by this library.
/// * **Error** – `is_error == 1` and `error_message` contains a descriptive C string allocated by
///   this library (also requiring the corresponding free function).
///
/// # Safety
/// * `model` **must be non-null** and point to a valid, NUL-terminated UTF-8 string that remains
///   alive for the entire duration of the call.
/// * `hf_token` may be null; if non-null it must satisfy the same UTF-8 and lifetime requirements
///   as `model`.
/// * Neither pointer may be mutated, reallocated, or freed by other threads while this function
///   executes.
/// * The returned `TokenizerHandle` is heap-allocated inside the library; it **must** be freed by
///   the caller using the provided destructor to avoid memory leaks.
///
...
///
/// # Safety
/// * **`tokenizer_handle` must be non-null** and point to a valid `TokenizerHandle` that remains
...
///
/// # Arguments
/// * `tokenizer_handle` – Pointer to a valid `TokenizerHandle` previously created by this library.
...
///   allocated by this library. The caller becomes responsible for freeing it using the appropriate
///   `free_string` function.
/// * **Error** – `is_error == 1` and `error_message` points to a descriptive C string, also
///   allocated by the library and requiring proper deallocation.
///
/// # Safety
/// * `tokenizer_handle` must be non-null and point to a valid `TokenizerHandle` that was created by
///   this library and is still valid.
...
/// * The caller must use the appropriate deallocation function to free the returned `string` or
///   `error_message` to avoid memory leaks.
///
