use crate::{err::Error, Subquery};
use crate::{Datetime, Duration, Idiom, Query, Range, Thing, Value};
use nom::{Err, Finish};
...
mod tests {
	...
	fn json_benchmark() {
		...
		// cargo test crate::parser::tests::json_benchmark --package surrealdb --lib --release -- --nocapture --exact
		...
		println!("crate::json took {:.10}s/iter", benchmark(|s| crate::json(s).unwrap()));
	}
