...
macro_rules! impl_module_def {
	...
	($ctx: expr, $path: literal, $name: literal, $call: ident, Async) => {
		{
            ...
            async fn f<'js>(ctx: js::Ctx<'js>, v: js::function::Rest<crate::value::Value>) -> js::Result<crate::value::Value>{
	...
	($ctx: expr, $path: literal, $name: literal, $call: ident, ) => {
		{
			let func = |ctx: js::Ctx<'_>, v: js::function::Rest<crate::value::Value>| $call(ctx,if $path == "" { $name } else { concat!($path, "::", $name) }, v.0);
			js::Function::new($ctx.clone(),func)?.with_name(stringify!($name))?
		}
	};
