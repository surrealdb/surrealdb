...
use crate::err::Error;
use crate::iam::{Action, Error as IamError, Resource};
...
impl Datastore {
	...
	pub async fn register_remove_and_archive(
		...
		let ts_expired = (&timestamp - &crate::duration::Duration::from_secs(5))?;
	...
	pub async fn remove_dead_nodes(
		...
		for hb in hbs {
			...
			nodes.push(crate::uuid::Uuid::from(hb.nd));
		}
		Ok(nodes)
	}
	...
	///
	/// ```rust,no_run
	/// use surrealdb_sql::kvs::{Datastore, TransactionType::*, LockType::*};
	/// use surrealdb_sql::err::Error;
	///
	/// #[tokio::main]
	/// async fn main() -> Result<(), Error> {
