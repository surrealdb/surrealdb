...
async fn expired_nodes_are_garbage_collected() {
	...
	test.db = test.db.with_node_id(crate::Uuid::from(old_node));
	...
	test.db = test.db.with_node_id(crate::Uuid::from(new_node));
	test.db.bootstrap().await.unwrap();
...
async fn expired_nodes_get_live_queries_archived() {
	...
	// Set up the first node at an early timestamp
	test.db = test.db.with_node_id(crate::Uuid::from(old_node)).with_notifications();
	test.db.bootstrap().await.unwrap();
	...
	let lq = LiveStatement {
		id: crate::Uuid(Uuid::parse_str("da60fa34-902d-4110-b810-7d435267a9f8").unwrap()),
		node: crate::uuid::Uuid::from(old_node),
		expr: Fields(vec![crate::Field::All], false),
		what: Table(crate::Table::from(table)),
		cond: None,
		fetch: None,
		archived: Some(crate::uuid::Uuid::from(old_node)),
...
async fn single_live_queries_are_garbage_collected() {
	...
	test.db = test.db.with_node_id(crate::uuid::Uuid::from(node_id));
	test.db.bootstrap().await.unwrap();
	...
	let live_st = LiveStatement {
		id: crate::Uuid(live_query_to_delete),
		node: crate::uuid::Uuid::from(node_id),
		expr: Fields(vec![crate::Field::All], false),
		what: Table(crate::Table::from(table)),
		cond: None,
		fetch: None,
	...
	let live_st = LiveStatement {
		id: crate::Uuid(live_query_to_keep),
		node: crate::Uuid::from(node_id),
		expr: Fields(vec![crate::Field::All], false),
		what: Table(crate::Table::from(table)),
		cond: None,
		fetch: None,
	...
	assert_eq!(scanned.len(), 1);
	assert_eq!(&scanned[0].lq, &crate::Uuid::from(live_query_to_keep));
	tx.commit().await.unwrap();
}
...
async fn bootstrap_does_not_error_on_missing_live_queries() {
	...
	let live_st = LiveStatement {
		id: crate::Uuid(live_query_to_corrupt),
		node: crate::uuid::Uuid::from(old_node_id),
		expr: Fields(vec![crate::Field::All], false),
		what: Table(crate::Table::from(table)),
		cond: None,
		fetch: None,
	...
	let second_node = test.db.with_node_id(crate::uuid::Uuid::from(new_node_id));
