...
async fn archive_lv_for_node_archives() {
	...
	let lv_id = crate::uuid::Uuid::from(Uuid::from_bytes([
	...
	let this_node_id = crate::uuid::Uuid::from(Uuid::from_bytes([
	...
	let results = test
		.db
		.archive_lv_for_node(&mut tx, &crate::uuid::Uuid(node_id), this_node_id)
		.await
		.unwrap();
	...
	assert_eq!(lq.nd, crate::uuid::Uuid(node_id));
