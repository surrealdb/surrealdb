...
async fn function_info_structure() {
	...
	let out_expected =
		r#"{ accesses: [], analyzers: [], configs: [], functions: [{ args: [['name', 'string']], as_roles: [], block: "{ RETURN 'Hello, ' + $name + '!'; }", name: 'example', permissions: true, returns: 'string' }], models: [], params: [], tables: [], users: [] }"#.to_string();
	let out_str = out.unwrap().to_string();
	assert_eq!(
		out_str, out_expected,
		"Output '{out_str}' doesn't match expected output '{out_expected}'",
