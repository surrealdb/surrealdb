...
```bash
cargo run --no-default-features --features storage-mem,http,scripting -- start --log trace memory
```

To listen to code changes as you develop, use the following command:

```bash
cargo watch -x 'run --no-default-features --features storage-mem,http,scripting -- start --log trace memory'
```

SurrealDB runs by default on port 8000. To change the default port, use the following command:

```bash
cargo run --no-default-features --features storage-mem,http,scripting -- start --log trace --bind 0.0.0.0:9000 memory
```
