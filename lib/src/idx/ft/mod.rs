...
mod tests {
	...
	pub(super) async fn tx_fti(
		...
		let write = matches!(store_type, StoreType::Write);
	...
	async fn test_ft_index() {
		...
		{
			// Add one document
			let (mut tx, mut fti) = tx_fti(&ds, StoreType::Write, &az, btree_order, false).await;
		...
		{
			let (mut tx, fti) = tx_fti(&ds, StoreType::Read, &az, btree_order, false).await;
	...
	async fn test_ft_index_bm_25(hl: bool) {
		...
		for _ in 0..10 {
			...
			let btree_order = 5;
			{
				let (mut tx, mut fti) = tx_fti(&ds, StoreType::Write, &az, btree_order, hl).await;
				fti.index_document(
					&mut tx,
					&doc1,
			...
			{
				let (mut tx, fti) = tx_fti(&ds, StoreType::Read, &az, btree_order, hl).await;
