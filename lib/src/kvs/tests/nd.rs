...
async fn write_scan_nd() {
	let nd = Uuid::from_str("6a6a4e59-3e86-431d-884f-8f433781e4e9").unwrap();
	...
	tx.set_nd(uuid::Uuid::parse_str("83d9b3c0-f3c4-45be-9ef9-9d48502fecb1").unwrap())
		.await
		.unwrap();
	tx.set_nd(uuid::Uuid::parse_str("cbefc4fe-8ba0-4898-ab69-782e3ebc06f9").unwrap())
		.await
		.unwrap();
