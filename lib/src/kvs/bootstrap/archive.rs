...
use crate::kvs::{ds, BootstrapOperationResult, Transaction};
use crate::sql::Uuid;
...
async fn retriable_transaction<F, R>(tx_req: mpsc::Sender<TxRequestOneshot>, retriable: F) -> Result<R, Error>
where
	F: Fn(Transaction) -> dyn Future<Output=Result<R, Error>> + Send + Sync + 'static,
{
	...
	for _ in 0..ds::BOOTSTRAP_TX_RETRIES {
		...
		match tx_res_oneshot.await {
			...
			Err(_recv_error) => {
				// TODO record that transaction not received
			}
		}
	}
	...
	return Err(Error::Internal("Retriable should have returned a value but reached end of loop without an error".to_string()))
}
...
async fn archive_live_query_batch(
	...
	let retriable_tx_result = retriable_transaction(tx_req, |mut tx| async move {
		...
		return match tx.commit().await {
			...
			Err(err) => {
				trace!("archive committing tx err {:?}", err);
				// Rehydrate the msg vector because of retries
				for processed in ret {
					msg.push(processed);
				}
				match tx.cancel().await {
					Ok(_) => { Err(err) }
					Err(e2) => {
						trace!("archive cancelling tx err {:?}", e2);
						Err(e2)
					}
				}
			}
