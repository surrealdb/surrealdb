...
impl<'a> Document<'a> {
	...
	pub async fn ft(
		...
		txn.clone().lock().unwrap().all_tb_views(opt.ns(), opt.db(), &id.tb).await
	}
	...
	pub async fn ev(
		...
		txn.clone().lock().unwrap().all_tb_events(opt.ns(), opt.db(), &id.tb).await
	}
	...
	pub async fn fd(
		...
		txn.clone().lock().unwrap().all_tb_fields(opt.ns(), opt.db(), &id.tb).await
	}
	...
	pub async fn ix(
		...
		txn.clone().lock().unwrap().all_tb_indexes(opt.ns(), opt.db(), &id.tb).await
	}
	...
	pub async fn lv(
		...
		let id = self.id.as_ref().unwrap();
		// Get the table definition
		txn.clone().lock().unwrap().all_tb_lives(opt.ns(), opt.db(), &id.tb).await
	}
}
