...
// Id is a unique monotonic id generated by the sequence generator.
pub(crate) type Id = u64;

// Seq is a sequence generator that can be used to generate unique monotonic ids.
...
impl Seq {
	...
	pub(crate) fn remove_id(&mut self, id: u64) {
		if let Some(available_ids) = &mut self.available_ids {
...
mod tests {
	...
	async fn get_ids(ds: &Datastore) -> (Transaction, Seq) {
		let mut tx = ds.transaction(true, false).await.unwrap();
		let key = "foo";
		let v = tx.get(key).await.unwrap();
		let d = Seq::new(key.into(), v).await.unwrap();
		(tx, d)
	}

	async fn finish(mut tx: Transaction, mut d: Seq) -> Result<(), Error> {
		match d.finish() {
			Some((key, val)) => {
