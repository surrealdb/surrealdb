...
mod fetch {
	...
	async fn fetch_impl<'js>(ctx: Ctx<'js>, args: Rest<()>) -> Result<()> {
		Exception::throw_internal(ctx,"The 'fetch' function is not available in this build of SurrealDB. In order to use 'fetch', enable the 'http' feature.")
	}
}
...
mod fetch {
	...
	use super::super::{
		...
		classes::{
			self, HeadersClass, RequestClass, RequestInit, ResponseClass, ResponseInit,
			ResponseType,
		},
		RequestError,
	};
