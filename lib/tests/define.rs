...
async fn define_remove_access() -> Result<(), Error> {
	...
	t.expect_val("None")?;
	t.expect_error(
		"The access method 'example' already exists in the database 'test' in namespace 'test'",
	)?;
	t.skip_ok(1)?;
	t.expect_error(
		"The access method 'example' does not exist in the database 'test' in namespace 'test'",
	)?;
	t.expect_val("None")?;
	Ok(())
}
