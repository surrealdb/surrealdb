...
async fn live_query_with_permission_gets_updates() {
	...
	let live_query_id: Option<Value::Uuid()> =
		db.query(format!("LIVE SELECT * FROM {table}")).await.unwrap().take(0).await.unwrap();
	assert_ne!(live_query_id, None);

	let mut some_data = HashMap::new();
	some_data.insert("some_key", "some_value");
	db.query(format!("INSERT INTO {table} {}", Value::Object(Object::from(some_data))))
		.await
		.unwrap();
