...
#[tokio::test]
async fn function_semver_compare() -> Result<(), Error> {
	let sql = r#"
		RETURN semver::compare("1.2.3", "1.0.0");
		RETURN semver::compare("1.2.3", "1.2.3");
		RETURN semver::compare("1.0.0", "1.2.3");
	"#;
	let dbs = new_ds().await?;
	let ses = Session::owner().with_ns("test").with_db("test");
	let res = &mut dbs.execute(sql, &ses, None).await?;
...
#[tokio::test]
async fn function_semver_extract() -> Result<(), Error> {
	let sql = r#"
		RETURN semver::major("1.2.3");
		RETURN semver::minor("1.2.3");
		RETURN semver::patch("1.2.3");
	"#;
	let dbs = new_ds().await?;
	let ses = Session::owner().with_ns("test").with_db("test");
	let res = &mut dbs.execute(sql, &ses, None).await?;
...
#[tokio::test]
async fn function_semver_increment() -> Result<(), Error> {
	let sql = r#"
		RETURN semver::increment::major("1.2.3");
		RETURN semver::increment::minor("1.2.3");
		RETURN semver::increment::patch("1.2.3");
	"#;
	let dbs = new_ds().await?;
	let ses = Session::owner().with_ns("test").with_db("test");
	let res = &mut dbs.execute(sql, &ses, None).await?;
	...
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'2.0.0'");
	assert_eq!(tmp, val);
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'1.3.0'");
	assert_eq!(tmp, val);
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'1.2.4'");
	assert_eq!(tmp, val);
	//
	Ok(())
}
...
#[tokio::test]
async fn function_semver_set() -> Result<(), Error> {
	let sql = r#"
		RETURN semver::set::major("1.2.3", 9);
		RETURN semver::set::minor("1.2.3", 9);
		RETURN semver::set::patch("1.2.3", 9);
	"#;
	let dbs = new_ds().await?;
	let ses = Session::owner().with_ns("test").with_db("test");
	let res = &mut dbs.execute(sql, &ses, None).await?;
	...
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'9.2.3'");
	assert_eq!(tmp, val);
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'1.9.3'");
	assert_eq!(tmp, val);
	//
	let tmp = res.remove(0).result?;
	let val = Value::parse("'1.2.9'");
	assert_eq!(tmp, val);
	//
	Ok(())
}
