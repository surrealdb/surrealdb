...
use parse::Parse;
mod helpers;
use crate::helpers::{dbs_test_execute, expected_values};
use helpers::new_ds;
...
async fn function_array_add() -> Result<(), Error> {
	...
	"#;
	let (_, _, mut res) = dbs_test_execute(sql).await?;
	assert_eq!(res.len(), 5);
	//
...
async fn function_math_abs() -> Result<(), Error> {
	...
	"#;
	let (_, _, mut res) = dbs_test_execute(sql).await?;
	expected_values(&mut res, vec![0, 100, 100])?;
	Ok(())
}
...
async fn function_math_pow() -> Result<(), Error> {
	...
	"#;
	let (dbs, ses, mut res) = dbs_test_execute(sql).await?;
	assert_eq!(res.len(), 2);
	//
	let tmp = res.remove(0).result?;
