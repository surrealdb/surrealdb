...
These general guidelines apply to every pull request. The Security Review Guide
below provides detailed invariants and review triggers for security-sensitive
subsystems.
...
- **Security**: credentials and tokens must not appear in logs or error messages. New endpoints and operations must enforce authentication and authorization checks. User input must be validated before reaching the storage layer. See the Security Review Guide for detailed invariants.
...
This document provides security-critical review guidance for SurrealDB. It describes
the invariants that must always hold, the patterns that require scrutiny, and the
areas where changes demand careful security analysis. Reviewers and AI agents should
use this as a checklist when evaluating changes to security-sensitive subsystems.
...
**Files**: `iam/`, `dbs/options.rs` (`Options` struct), document processing pipeline,
`check_perms`, `check_permissions_table`, `process_table_fields`, `pluck_generic`
