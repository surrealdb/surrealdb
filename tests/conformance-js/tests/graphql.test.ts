...
// GraphQL endpoint (/graphql + the `graphql` RPC method) conformance.
//
// GraphQL is available by default — no capability or experimental flag. The only
// gate is the per-database `DEFINE CONFIG GRAPHQL AUTO` (or `TABLES INCLUDE ...`),
// without which every request is HTTP 400. The route and RPC method can be shut
// off with `--deny-http=graphql` and `--deny-rpc=graphql`.
//
// Coverage: schema generation and introspection (SCHEMAFULL vs SCHEMALESS,
// aliases, name collisions, descriptions, depth/complexity limits, cache
// invalidation on DDL), the auto-generated CRUD surface (lists, filtering and
// ordering, cursor pagination, aggregates, relations, record links, functions),
// mutations and their PERMISSIONS enforcement, signIn/signUp auth mutations,
// error-message safety, the `graphql` RPC method, and interop with standard
// GraphQL clients (graphql-request over HTTP, graphql-ws subscriptions).
//
// The suite mixes drivers deliberately: raw fetch, RpcClient, graphql-request
// and graphql-ws all exercise the same endpoint.
//
// Fixtures: a group of baseline tests share one server (beforeAll/afterAll); the
// remaining tests each spawn their own server (some with custom flags) and tear
// it down per-test. Every test uses a unique ns/db, closes every client, and
// stops its server.
...
/** Fresh server + unique ns/db with GraphQL AUTO and a seeded `book` table. */
async function seedBooks(): Promise<{
	...
	await db.query(`
		...
		DEFINE FIELD year ON book TYPE int;
		CREATE book:one SET title = 'A', year = 1999;
		CREATE book:two SET title = 'B', year = 2005;
		CREATE book:three SET title = 'C', year = 2020;
	`);
	const headers = {
		"surreal-ns": namespace,
		"surreal-db": database,
...
// ---------------------------------------------------------------------------
// (7) STANDARD GRAPHQL CLIENT INTEROP — graphql-request (HTTP) + graphql-ws (subs)
// ---------------------------------------------------------------------------

test(
	"graphql-request: HTTP list query returns the seeded records in order",
	async () => {
		const { server, headers } = await seedBooks();
		const client = new GraphQLClient(`${server.httpUrl}/graphql`, { headers });

		// The auto-schema exposes the pluralized `books` list field, a singular
		// `book(id:)` lookup, `books_aggregate`, and `booksConnection`.
		const data = await client.request<{
			books: Array<{ id: string; title: string; year: number }>;
		}>("{ books(order: {asc: year}) { id title year } }");

		expect(data.books).toEqual([
			{ id: "book:one", title: "A", year: 1999 },
			{ id: "book:two", title: "B", year: 2005 },
			{ id: "book:three", title: "C", year: 2020 },
		]);
	},
	TIMEOUT,
);
...
test(
	"graphql-request: HTTP query with GraphQL variables (limit + order)",
	async () => {
		const { server, headers } = await seedBooks();
		const client = new GraphQLClient(`${server.httpUrl}/graphql`, { headers });
		...
		const query = `
			query Books($limit: Int!) {
				books(limit: $limit, order: {asc: year}) { title }
			}
		`;
		const data = await client.request<{ books: Array<{ title: string }> }>(query, { limit: 2 });

		expect(data.books).toEqual([{ title: "A" }, { title: "B" }]);
	},
	TIMEOUT,
);

test(
	"graphql-request: HTTP singular lookup by bare record key",
	async () => {
		const { server, headers } = await seedBooks();
		const client = new GraphQLClient(`${server.httpUrl}/graphql`, { headers });

		const data = await client.request<{ book: { id: string; title: string } | null }>(
			'{ book(id: "two") { id title } }',
		);

		expect(data.book).toEqual({ id: "book:two", title: "B" });
	},
	TIMEOUT,
);

test(
	"graphql-ws: subscription streams a live-query event over graphql-transport-ws",
	async () => {
		const { server, db, headers } = await seedBooks();

		// graphql-ws instantiates `new webSocketImpl(url, subprotocol)`. SurrealDB
		// authenticates the WS *upgrade* from headers (ns/db + auth), not the
		// connection_init payload, so inject them on the underlying `ws` socket.
		const client = createWsClient({
			url: `ws://127.0.0.1:${server.port}/graphql`,
			webSocketImpl: class extends NodeWebSocket {
		...
		type BookEvent = { book: { id: string; title: string; year: number } };
		const delivered = new Promise<BookEvent["book"]>((resolve, reject) => {
			const unsubscribe = client.subscribe<BookEvent>(
				{ query: "subscription { book { id title year } }" },
				{
					next: ({ data }) => {
						// Live queries only stream changes registered after subscribe,
						// so the three seeded rows never arrive — only book:live does.
						if (data?.book?.id === "book:live") {
							unsubscribe();
							resolve(data.book);
						}
					},
		...
		// Let the server register the live query before mutating.
		await Bun.sleep(1000);
		await db.query("CREATE book:live SET title = 'Live', year = 2026");

		const event = await Promise.race([
			delivered,
			new Promise<never>((_, reject) =>
				setTimeout(() => reject(new Error("no subscription event within 10s")), 10000),
			),
		]);

		expect(event).toEqual({ id: "book:live", title: "Live", year: 2026 });
	},
	TIMEOUT,
);
