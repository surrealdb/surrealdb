...
mod cli_integration {
	...
	async fn all_commands() {
		...
		info!("* Export to stdout");
		{
			...
			let output =
				common::run(&args).output().expect(&format!("failed to run stdout export: {args}"));
		...
		let exported = {
			...
			common::run(&args).output().expect(&format!("failed to run file export: {args}"));
			exported
		};
		...
		info!("* Import the exported file");
		{
			...
			common::run(&args).output().expect(&format!("failed to run import: {args}"));
		}
