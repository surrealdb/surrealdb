...
use surrealdb_core::rpc::format::cbor::encode::Encode;
	...
	fn to_msg(format: Format, message: &serde_json::Value) -> Result<Message> {
		match format {
			...
			Format::Cbor => {
				...
				surrealql.encode(&mut writer).unwrap();
				// THen output the message.
				Ok(Message::Binary(writer.into_inner()))
			}
		}
	}
			...
			Message::Binary(msg) => {
				...
				match format {
					Format::Cbor => {
						...
						let mut dec = Decoder::new_from_slice(msg.as_slice());
						let msg: SqlValue = dec.decode()?;
