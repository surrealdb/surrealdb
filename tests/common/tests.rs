use super::common::{self, DB, NS, PASS, USER};
use common::Format;
use common::Socket;
use serde_json::json;
...
async fn ping() -> Result<(), Box<dyn std::error::Error>> {
	...
	let mut socket = Socket::connect_with_format(&addr, SERVER).await?;
	// Send INFO command
	let res = socket
...
async fn format() -> Result<(), Box<dyn std::error::Error>> {
	...
	socket.send_message_query(FORMAT, "CREATE table:id").await?;
	...
	socket
		...
		.await?;
	// Query data
	let res = socket.send_message_query(Format::Json, "SELECT * FROM table:id").await?;
	assert!(res[0]["result"].is_array(), "result: {:?}", res);
	assert_eq!(res[0]["result"].as_array().unwrap().len(), 1, "result: {:?}", res);
	// Change format to CBOR
	socket
		.send_and_receive_message(
			Format::Json,
			json!({
		...
		.await?;
	// Query data
	let res = socket.send_message_query(Format::Cbor, "SELECT * FROM table:id").await?;
	assert!(res[0]["result"].is_array(), "result: {:?}", res);
	assert_eq!(res[0]["result"].as_array().unwrap().len(), 1, "result: {:?}", res);
	// Change format to MsgPack
	socket
		.send_and_receive_message(
			Format::Cbor,
			json!({
		...
		.await?;
	// Query data
	let res = socket.send_message_query(Format::Pack, "SELECT * FROM table:id").await?;
	assert!(res[0]["result"].is_array(), "result: {:?}", res);
	assert_eq!(res[0]["result"].as_array().unwrap().len(), 1, "result: {:?}", res);
	// Change format to original format
	socket
		.send_and_receive_message(
			Format::Pack,
			json!({
		...
		.await?;
	// Query data
	let res = socket.send_message_query(FORMAT, "SELECT * FROM table:id").await?;
	assert!(res[0]["result"].is_array(), "result: {:?}", res);
	assert_eq!(res[0]["result"].as_array().unwrap().len(), 1, "result: {:?}", res);
	// Test passed
	Ok(())
...
async fn live_second_connection() -> Result<(), Box<dyn std::error::Error>> {
	...
	let (addr, _server) = common::start_server_with_defaults().await.unwrap();
	// Connect to WebSocket
	let mut socket1 = Socket::connect_with_format(&addr, SERVER).await?;
	// Authenticate the connection
	...
	// Connect to WebSocket
	let mut socket2 = Socket::connect_with_format(&addr, SERVER).await?;
	// Authenticate the connection
