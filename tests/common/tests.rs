...
async fn ping() -> Result<(), Box<dyn std::error::Error>> {
	retry::retry(Fixed::from_millis(100).take(5), || async {
...
async fn live() -> Result<(), Box<dyn std::error::Error>> {
	retry::retry(Fixed::from_millis(100).take(5), || async {
		...
		let res =
			socket.send_request("query", json!(["CREATE tester:id SET name = 'foo'"])).await?;
		assert!(res.is_object(), "result: {:?}", res);
		assert!(res["result"].is_array(), "result: {:?}", res);
		let res = res["result"].as_array().unwrap();
		assert_eq!(res.len(), 1, "result: {:?}", res);
		...
		let msgs: Result<_, Box<dyn std::error::Error>> =
			tokio::time::timeout(Duration::from_secs(1), async {
				Ok(vec![
					socket.receive_other_message().await?,
					socket.receive_other_message().await?,
				])
			})
			.await?;
...
async fn kill() -> Result<(), Box<dyn std::error::Error>> {
	retry::retry(Fixed::from_millis(100).take(5), || async {
		...
		// Create a new test record
		let res =
			socket.send_request("query", json!(["CREATE tester:one SET name = 'one'"])).await?;
		assert!(res.is_object(), "result: {:?}", res);
		assert!(res["result"].is_array(), "result: {:?}", res);
		let res = res["result"].as_array().unwrap();
		assert_eq!(res.len(), 1, "result: {:?}", res);
		...
		// Create a new test record
		let res =
			socket.send_request("query", json!(["CREATE tester:two SET name = 'two'"])).await?;
		assert!(res.is_object(), "result: {:?}", res);
		assert!(res["result"].is_array(), "result: {:?}", res);
		let res = res["result"].as_array().unwrap();
		assert_eq!(res.len(), 1, "result: {:?}", res);
		...
		// Create a new test record
		let res =
			socket.send_request("query", json!(["CREATE tester:tre SET name = 'two'"])).await?;
		assert!(res.is_object(), "result: {:?}", res);
		assert!(res["result"].is_array(), "result: {:?}", res);
		let res = res["result"].as_array().unwrap();
		assert_eq!(res.len(), 1, "result: {:?}", res);
...
async fn live_second_connection() -> Result<(), Box<dyn std::error::Error>> {
	retry::retry(Fixed::from_millis(100).take(5), || async {
		...
		// Create a new test record
		let res =
			socket2.send_request("query", json!(["CREATE tester:id SET name = 'foo'"])).await?;
		assert!(res.is_object(), "result: {:?}", res);
		assert!(res["result"].is_array(), "result: {:?}", res);
		let res = res["result"].as_array().unwrap();
		assert_eq!(res.len(), 1, "result: {:?}", res);
