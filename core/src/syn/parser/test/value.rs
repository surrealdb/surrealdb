...
#[test]
fn parse_coordinate() {
	test_parse!(parse_value_class, "(1.88, -18.0)").unwrap();
}

#[test]
fn parse_like_operator() {
	test_parse!(parse_value_class, "a ~ b").unwrap();
}
...
fn parse_recursive_record_string() {
	let res = test_parse!(parse_value_class, r#" r"a:[r"b:{c: r"d:1"}"]" "#).unwrap();
	assert_eq!(
		res,
		Value::Thing(Thing {
			tb: "a".to_owned(),
...
fn parse_record_string_2() {
	let res = test_parse!(parse_value_class, r#" r'a:["foo"]' "#).unwrap();
	assert_eq!(
		res,
		Value::Thing(Thing {
			tb: "a".to_owned(),
...
#[test]
fn parse_i64() {
	let res = test_parse!(parse_value_class, r#" -9223372036854775808 "#).unwrap();
	assert_eq!(res, Value::Number(Number::Int(i64::MIN)));

	let res = test_parse!(parse_value_class, r#" 9223372036854775807 "#).unwrap();
	assert_eq!(res, Value::Number(Number::Int(i64::MAX)));
}

#[test]
fn constant_lowercase() {
	let out = test_parse!(parse_value_class, r#" math::pi "#).unwrap();
	assert_eq!(out, Value::Constant(Constant::MathPi));

	let out = test_parse!(parse_value_class, r#" math::inf "#).unwrap();
	assert_eq!(out, Value::Constant(Constant::MathInf));

	let out = test_parse!(parse_value_class, r#" math::neg_inf "#).unwrap();
...
fn scientific_decimal() {
	let res = test_parse!(parse_value_class, r#" 9.7e-7dec "#).unwrap();
...
fn scientific_number() {
	let res = test_parse!(parse_value_class, r#" 9.7e-5"#).unwrap();
...
#[test]
fn empty_string() {
	test_parse!(parse_value_class, "").unwrap_err();
}
