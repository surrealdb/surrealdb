...
impl Parser<'_> {
	...
	async fn parse_object_or_geometry_after_type(
		...
		match type_value.as_str() {
			"Point" => {
				...
				self.parse_geometry_after_type(ctx, start, key, type_value, Value::to_point, |x| {
					Value::Geometry(Geometry::Point(x))
				})
				.await
			}
			"LineString" => {
				self.parse_geometry_after_type(ctx, start, key, type_value, Value::to_line, |x| {
	...
	async fn parse_object_or_geometry_after_coordinates(
		...
		match type_value.as_str() {
			"Point" => {
				if self.eat(t!("}")) {
					if let Some(point) = Value::to_point(&value) {
			...
			"LineString" => {
				if self.eat(t!("}")) {
					if let Some(point) = Value::to_line(&value) {
			...
			"Polygon" => {
				if self.eat(t!("}")) {
					if let Some(point) = Value::to_polygon(&value) {
			...
			"MultiPoint" => {
				if self.eat(t!("}")) {
					if let Some(point) = Value::to_multipolygon(&value) {
			...
			"MultiLineString" => {
				if self.eat(t!("}")) {
					if let Some(point) = Value::to_multiline(&value) {
