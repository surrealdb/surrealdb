...
mod tests {
	...
	fn check_create_hnsw_index() {
		let sql = "INDEX my_index ON TABLE my_table COLUMNS my_col HNSW DIMENSION 4 HEURISTIC EXTEND_CANDIDATES KEEP_PRUNED_CONNECTIONS";
		let (_, idx) = index(sql).unwrap();
		assert_eq!(
			idx,
			DefineIndexStatement {
		...
		assert_eq!(
			idx.to_string(),
			"DEFINE INDEX my_index ON my_table FIELDS my_col HNSW DIMENSION 4 DIST EUCLIDEAN TYPE F64 EFC 150 M 12 M0 24 ML 0.40242960438184466f HEURISTIC EXTEND_CANDIDATES KEEP_PRUNED_CONNECTIONS"
		);
	}
