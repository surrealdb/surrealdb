...
impl<'a> Document<'a> {
					...
					match &tb.cond {
						...
						Some(cond) => {
							...
							if !targeted_force
								...
								.compute(ctx, opt, txn, Some(&self.initial))
								.await?
								.is_truthy()
							{
								...
								self.data(ctx, opt, txn, fdc).await?;
							}
									...
									match act {
										...
										_ => {
											let stm = UpdateStatement {
												what: Values(vec![Value::from(rid)]),
												data: Some(
													self.full(ctx, opt, txn, &tb.expr).await?,
												),
	...
	async fn get_group_ids(
		...
		Ok(try_join_all(group.iter().map(|v| v.compute(ctx, opt, txn, Some(doc))))
			.await?
			.into_iter()
			.collect::<Vec<_>>())
	}
	...
	async fn data(
		...
		//
		let (set_ops, del_ops) = self.fields(ctx, opt, txn, &fdc).await?;
		//
		let thg = Thing {
			...
			} = field
			{
				...
				match expr {
					Value::Function(f) if f.is_rolling() => match f.name() {
						Some("count") => {
							let val = f.compute(ctx, opt, txn, Some(fdc.doc)).await?;
							self.chg(&mut set_ops, &mut del_ops, &fdc.act, idiom, val);
						}
						Some("math::sum") => {
							let val = f.args()[0].compute(ctx, opt, txn, Some(fdc.doc)).await?;
							self.chg(&mut set_ops, &mut del_ops, &fdc.act, idiom, val);
						}
					...
					_ => {
						let val = expr.compute(ctx, opt, txn, Some(fdc.doc)).await?;
