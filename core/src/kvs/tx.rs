...
impl Transaction {
	...
	pub async fn get_record(
		...
		let key = crate::key::thing::new(ns, db, tb, &id.to_owned().try_into()?).encode()?;
		let res = self.cache.get_value_or_guard_async(&key).await;
		match res {
	...
	pub async fn set_record(
		...
		let key = crate::key::thing::new(ns, db, tb, &id.to_owned().try_into()?);
		let enc = crate::key::thing::new(ns, db, tb, &id.to_owned().try_into()?).encode()?;
