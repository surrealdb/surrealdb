...
pub static MATCHER: Lazy<SkimMatcherV2> = Lazy::new(|| SkimMatcherV2::default().ignore_case());
...
pub fn json(i: &str) -> IResult<&str, Value> {
	alt((
		...
		map(tag_no_case("true"), |_| Value::True),
		map(tag_no_case("false"), |_| Value::False),
		map(datetime_raw, |v| Value::Datetime(v)),
		map(duration_raw, |v| Value::Duration(v)),
		map(number, |v| Value::Number(v)),
		map(object, |v| Value::Object(v)),
		map(array, |v| Value::Array(v)),
		map(strand, |v| Value::Strand(v)),
