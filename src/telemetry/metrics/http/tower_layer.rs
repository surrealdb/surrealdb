use std::{task::{Context, Poll}, time::{Duration, Instant}, cell::Cell, pin::Pin, fmt};
use axum::extract::MatchedPath;
use opentelemetry::{KeyValue, metrics::MetricsError, Context as TelemetryContext};
use pin_project_lite::pin_project;

use futures::Future;
use http::{Request, Version, StatusCode, Response};
...
#[derive(Clone)]
pub struct HttpMetrics<S> { inner: S }
...
impl HttpCallMetricTracker {
    ...
    pub(super) fn request_duration_attrs(&self) -> Vec<KeyValue> {
        ...
        res.push(KeyValue::new("http.response.status_code", self.status_code.map(|v| v.as_str().to_owned()).unwrap_or("000".to_owned())));
