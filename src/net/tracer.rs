...
use http::{header, StatusCode, request::Parts};
use hyper::{Request, Response};
use axum::{body::{boxed, Body, BoxBody}, extract::MatchedPath, RequestPartsExt, headers::{Authorization, authorization::{Basic, Bearer}, Origin}, TypedHeader, Extension};
use surrealdb::{dbs::Session, iam::{verify::token}};
use tower_http::{auth::AsyncAuthorizeRequest, trace::{MakeSpan, OnRequest, OnResponse, OnFailure}, request_id::RequestId};
...
impl<B> MakeSpan<B> for HttpTraceLayerHooks {
    fn make_span(&mut self, req: &Request<B>) -> Span {
        ...
        req.headers().get(header::CONTENT_LENGTH).map(|v| v.to_str().map(|v| span.record("http.request.body.size", v)));
        req.headers().get(header::USER_AGENT).map(|v| v.to_str().map(|v| span.record("user_agent.original", v)));
        ...
        if let Some(req_id) = req.extensions().get::<RequestId>() {
            match req_id.header_value().to_str() {
                ...
                Ok(request_id) => { span.record("http.request.id", request_id); },
            }
        }
