...
impl Results {
	...
	pub(in crate::dbs) async fn push(
		&mut self,
		stk: &mut Stk,
		ctx: &Context,
		opt: &Options,
	...
	pub(in crate::dbs) async fn sort(&mut self, orders: &Ordering) -> Result<()> {
		match self {
			#[cfg(storage)]
	...
	pub(in crate::dbs) fn is_empty(&self) -> bool {
		self.len() == 0
	}
	...
	pub(in crate::dbs) async fn take(&mut self) -> Result<Vec<Value>> {
		Ok(match self {
