...
use crate::protocol::surrealdb::rpc::{DatabaseAccessCredentials, NamespaceAccessCredentials, NamespaceUserCredentials, SignupParams};
use crate::protocol::surrealdb::rpc::Access as AccessProto;
...
pub async fn signup(kvs: &Datastore, session: &mut Session, SignupParams {
	namespace,
	database,
	access,
	access_params,
}: SignupParams) -> Result<SignupData> {
		// Attempt to signup using specified access method
		// Currently, signup is only supported at the database level

		let access_params = access_params
			.into_iter()
			.map(|(k, v)| Ok((k, v.try_into()?)))
			.collect::<Result<BTreeMap<String, Value>>>()?;

		super::signup::db_access(kvs, session, namespace, database, access, access_params).await
}
...
pub async fn db_access(
	...
	let access_definition = tx.get_db_access(&namespace, &database, &access).await?;
	// Ensure that the transaction is cancelled
	tx.cancel().await?;

	// Check the access method type
	// Currently, only the record access method supports signup
	let AccessType::Record(ref at) = access_definition.kind else {
	...
	let mut sess = Session::editor().with_ns(&namespace).with_db(&database);
	...
	match kvs.evaluate(val, &sess, Some(access_params)).await {
		...
		Ok(val) => {
			...
			let claims = Claims {
				...
				exp: expiration(access_definition.duration.token)?,
				jti: Some(Uuid::new_v4().to_string()),
				ns: Some(namespace.clone()),
				db: Some(database.clone()),
				ac: Some(access.clone()),
				id: Some(rid.to_raw()),
				..Claims::default()
			};
			// AUTHENTICATE clause
			if let Some(au) = &access_definition.authenticate {
			...
			trace!("Signing up with access method `{access}`");
			...
			session.ac = Some(access.clone());
			session.rd = Some(Value::from(rid.clone()));
			session.exp = expiration(access_definition.duration.session)?;
			session.au = Arc::new(Auth::new(Actor::new(
				rid.to_string(),
				Default::default(),
				Level::Record(namespace, database, rid.to_string()),
			)));
