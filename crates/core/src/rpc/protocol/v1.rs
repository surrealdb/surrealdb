...
pub trait RpcProtocolV1: RpcContext {
	...
	async fn graphql(&self, _params: Array) -> Result<Data, RpcError> {
		...
		// return Err(RpcError::InvalidParams(format!("Expected (query, options)
		// got {:?}", params.0))); };
		//
		// enum GraphQLFormat {
		// Json,
		...
		// _ => return Err(RpcError::InvalidParams(format!("Expected (query,
		// options) got {:?}", params.0))), },
		// _ => return Err(RpcError::InvalidParams(format!("Expected (query,
		// options) got {:?}", params.0))), }
		// }
		// }
		...
		// }
		// Some(_) => return Err(RpcError::InvalidParams(format!("Expected
		// (query, options) got {:?}", params.0))), None => {}
		// }
