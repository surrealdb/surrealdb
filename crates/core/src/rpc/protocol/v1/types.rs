...
impl From<crate::dbs::QueryType> for QueryType {
	fn from(query_type: crate::dbs::QueryType) -> Self {
		match query_type {
			crate::dbs::QueryType::Other => QueryType::Other,
			crate::dbs::QueryType::Live => QueryType::Live,
			crate::dbs::QueryType::Kill => QueryType::Kill,
		}
	}
}

impl From<QueryType> for crate::dbs::QueryType {
	fn from(query_type: QueryType) -> Self {
		match query_type {
			QueryType::Other => crate::dbs::QueryType::Other,
			QueryType::Live => crate::dbs::QueryType::Live,
			QueryType::Kill => crate::dbs::QueryType::Kill,
		}
	}
}
...
impl V1QueryResponse {
	...
	pub fn from_query_result(
		...
		let value = result.context("Query result is empty")?;
		let value = V1Value::try_from(value)?;
		Ok(Self {
			time: V1Duration(time),
			result: Ok(value),
...
impl V1Value {
	...
	pub fn kindof(&self) -> &'static str {
		...
		match self {
			...
			Self::Number(V1Number::Int(_)) => "int",
			Self::Number(V1Number::Float(_)) => "float",
			Self::Number(V1Number::Decimal(_)) => "decimal",
			Self::Geometry(V1Geometry::Point(_)) => "geometry<point>",
			Self::Geometry(V1Geometry::Line(_)) => "geometry<line>",
			Self::Geometry(V1Geometry::Polygon(_)) => "geometry<polygon>",
			Self::Geometry(V1Geometry::MultiPoint(_)) => "geometry<multipoint>",
			Self::Geometry(V1Geometry::MultiLine(_)) => "geometry<multiline>",
			Self::Geometry(V1Geometry::MultiPolygon(_)) => "geometry<multipolygon>",
			Self::Geometry(V1Geometry::Collection(_)) => "geometry<collection>",
			Self::Bytes(_) => "bytes",
			Self::Range(_) => "range",
			_ => "incorrect type",
		}
...
#[non_exhaustive]
pub struct V1Range {
	pub beg: Bound<V1Value>,
	pub end: Bound<V1Value>,
}
...
impl V1Array {
	...
	pub fn push(&mut self, value: V1Value) {
		self.0.push(value);
	}
}
...
impl V1Object {
	pub fn new(value: BTreeMap<String, V1Value>) -> Self {
		Self(value)
	}
}
...
impl std::ops::DerefMut for V1Object {
...
impl V1BindingPower {
	...
	fn for_value(value: &V1Value) -> V1BindingPower {
		match value {
			V1Value::Range(..) => V1BindingPower::Range,
			_ => V1BindingPower::Prime,
		}
	}
}

impl fmt::Display for V1Range {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match &self.beg {
			...
			Bound::Included(v) => {
				...
				if V1BindingPower::for_value(v) <= V1BindingPower::Range {
					write!(f, "({})", v)
				} else {
					write!(f, "{}", v)
				}
