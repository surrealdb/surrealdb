...
use crate::{
	...
	rpc::protocol::v1::types::{
		V1Array, V1Datetime, V1Duration, V1File, V1Model, V1Object, V1RecordId, V1String, V1Uuid,
		V1Value,
	},
};
use anyhow::Result;
...
pub(super) fn to_value(content: Content) -> Result<V1Value> {
	match content {
		Content::Struct(v) => match v.name.as_ref() {
			expr::strand::TOKEN => V1String::deserialize(Content::Struct(v).into_deserializer())
				.map(Into::into)
				.map_err(Into::into),
