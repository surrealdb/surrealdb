...
impl<'a> Reader<'a> {
    ...
    pub fn peek(&mut self) -> Result<u8, Error> {
        let Some(x) = self.0.get(self.1) else {
            return Err(Error::OutOfBounds { byte: self.1, len: self.0.len() })
        };
    ...
    pub fn read_u16(&mut self) -> Result<u16, Error> {
        let bytes = self.0.get(self.1..self.1 + 2).ok_or(Error::OutOfBounds { byte: self.1 + 1, len: self.0.len() })?;
        self.1 += 2;
        Ok(u16::from_le_bytes([bytes[0], bytes[1]]))
    }

    pub fn read_u32(&mut self) -> Result<u32, Error> {
        let bytes = self.0.get(self.1..self.1 + 4).ok_or(Error::OutOfBounds { byte: self.1 + 3, len: self.0.len() })?;
        self.1 += 4;
        Ok(u32::from_le_bytes(bytes.try_into().unwrap()))
    }

    pub fn read_u64(&mut self) -> Result<u64, Error> {
        let bytes = self.0.get(self.1..self.1 + 8).ok_or(Error::OutOfBounds { byte: self.1 + 7, len: self.0.len() })?;
        self.1 += 8;
    ...
    pub fn read_bytes(&mut self, len: usize) -> Result<&[u8], Error> {
        let bytes = self.0.get(self.1..self.1 + len).ok_or(Error::OutOfBounds { byte: self.1 + len - 1, len: self.0.len() })?;
        self.1 += len;
        Ok(bytes)
    }
