...
pub enum Error {
    ...
    #[error("Expected a chunk of major {expected}, but found major {found} instead while decoding an infinite value")]
    InvalidChunkMajor {
        found: u8,
        expected: u8,
    },
    ...
    #[error("Encountered an invalid simple simple value under tag 7, with length {0}")]
