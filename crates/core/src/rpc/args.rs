...
impl Take for Array {
	...
	fn needs_three_four_or_five(self) -> Result<(Value, Value, Value, Value, Value), RpcError> {
		...
		match (x.next(), x.next(), x.next(), x.next(), x.next()) {
			...
			(Some(a), Some(b), Some(c), Some(d), None) => Ok((a, b, c, d, Value::None)),
			(Some(a), Some(b), Some(c), None, None) => Ok((a, b, c, Value::None, Value::None)),
			(_, _, _, _, _) => {
				Ok((Value::None, Value::None, Value::None, Value::None, Value::None))
			}
		}
	}
}
