...
impl super::api::Transaction for Transaction {
	...
	async fn keys(
		...
		let res = affinitypool::spawn_local(|| -> Result<_> {
			...
			let res = match version {
				Some(ts) => inner
					.keys_at_timestamp(beg..end_clone, ts, Some(limit as usize))?
					.into_iter()
	...
	async fn scan(
		...
		let res = affinitypool::spawn_local(|| -> Result<_> {
			...
			let res = match version {
				Some(ts) => inner
					.scan_at_timestamp(beg..end_clone, ts, Some(limit as usize))?
					.into_iter()
					.map(|(k, v)| (k, v.to_vec()))
	...
	async fn scan_all_versions(
		...
		let res = affinitypool::spawn_local(|| -> Result<_> {
			// Retrieve the scan range
			let res = inner
				.scan_all_timestamps(beg..end, Some(limit as usize))?
				.into_iter()
