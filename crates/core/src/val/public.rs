...
use super::Number as InternalNumber;
use super::Datetime as InternalDatetime;
use super::Uuid as InternalUuid;
use super::Duration as InternalDuration;
use super::Bytes as InternalBytes;
use super::Object as InternalObject;
use super::Geometry as InternalGeometry;
use super::File as InternalFile;
use super::Range as InternalRange;
use super::RecordId as InternalRecordId;
...
use surrealdb_types::Datetime as PublicDatetime;
use surrealdb_types::Uuid as PublicUuid;
use surrealdb_types::Duration as PublicDuration;
use surrealdb_types::Bytes as PublicBytes;
use surrealdb_types::Object as PublicObject;
use surrealdb_types::Geometry as PublicGeometry;
use surrealdb_types::File as PublicFile;
use surrealdb_types::Range as PublicRange;
...
impl From<PublicGeometry> for InternalGeometry {
    fn from(g: PublicGeometry) -> Self {
        match g {
            ...
            PublicGeometry::Collection(c) => InternalGeometry::Collection(c.into_iter().map(|g| g.into()).collect()),
        }
    }
}
...
impl From<InternalGeometry> for PublicGeometry {
    fn from(g: InternalGeometry) -> Self {
        match g {
            ...
            InternalGeometry::Collection(c) => PublicGeometry::Collection(c.into_iter().map(|g| g.into()).collect()),
        }
    }
}
...
impl TryFrom<PublicArray> for InternalArray {
    ...
    fn try_from(a: PublicArray) -> anyhow::Result<Self> {
        Ok(InternalArray(a.0.into_iter().map(|v| v.try_into()).collect::<anyhow::Result<Vec<_>>>()?))
    }
}
...
impl TryFrom<PublicObject> for InternalObject {
    ...
    fn try_from(o: PublicObject) -> anyhow::Result<Self> {
        Ok(InternalObject(o.0.into_iter().map(|(k, v)| v.try_into().map(|v: InternalValue| (k, v))).collect::<anyhow::Result<BTreeMap<String, InternalValue>>>()?))
    }
}
...
impl TryFrom<InternalObject> for PublicObject {
    ...
    fn try_from(o: InternalObject) -> anyhow::Result<Self> {
        Ok(PublicObject(o.0.into_iter().map(|(k, v)| v.try_into().map(|v: PublicValue| (k, v))).collect::<anyhow::Result<BTreeMap<String, PublicValue>>>()?))
    }
}
...
impl TryFrom<PublicRecordIdKey> for InternalRecordIdKey {
    ...
    fn try_from(r: PublicRecordIdKey) -> anyhow::Result<Self> {
        match r {
            ...
            PublicRecordIdKey::Range(r) => Ok(InternalRecordIdKey::Range(Box::new((*r).try_into()?))),
        }
    }
}
...
impl TryFrom<InternalRecordIdKey> for PublicRecordIdKey {
    ...
    fn try_from(r: InternalRecordIdKey) -> anyhow::Result<Self> {
        match r {
            ...
            InternalRecordIdKey::Range(r) => Ok(PublicRecordIdKey::Range(Box::new((*r).try_into()?))),
        }
    }
}
