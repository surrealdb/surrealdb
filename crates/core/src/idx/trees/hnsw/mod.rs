	...
	L: DynamicSet,
{
	...
	async fn check_state(&mut self, ctx: &Context) -> Result<(), Error> {
		let tx = ctx.tx();
		// Read the state
		let mut st: HnswState = if let Some(val) = tx.get(self.state_key.clone(), None).await? {
...
mod tests {
	...
	async fn insert_collection_hnsw_index(
		...
		for (doc_id, obj) in collection.to_vec_ref() {
			...
			h.index_document(ctx, &Id::Number(*doc_id as i64), &content).await.unwrap();
	...
	async fn delete_hnsw_index_collection(
		...
		for (doc_id, obj) in collection.to_vec_ref() {
			let content = vec![Value::from(obj.deref())];
			h.remove_document(ctx, Id::Number(*doc_id as i64), &content).await?;
	...
	async fn test_recall(
		...
		for (doc_id, obj) in collection.to_vec_ref() {
			let content = vec![Value::from(obj.deref())];
			h.index_document(&ctx, &Id::Number(*doc_id as i64), &content).await?;
		}
		tx.commit().await?;
