...
impl<'a> StatementContext<'a> {
	...
	/// Decide whether to fetch just record keys, keys and values, or only a COUNT.
	///
	/// This function evaluates the statement shape (UPDATE/DELETE/etc.), WHERE/GROUP/ORDER
	/// clauses, selected fields, and table permissions to select the most efficient
	/// record retrieval strategy:
	/// - KeysAndValues: required when values must be read (e.g., UPDATE/DELETE; WHERE not fully
	///   covered by indexes; GROUP BY with fields; ORDER BY with fields; non-count projections;
	///   or when table permissions are Specific).
	...
	///
	/// On backends that support reverse scans (e.g., RocksDB/TiKV), we reverse the
	/// direction when the first ORDER BY is `id DESC`. Otherwise, we default to forward.
