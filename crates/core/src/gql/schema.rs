...
>>>>>>> main:crates/core/src/gql/schema.rs
...
pub async fn generate_schema(
	...
	match (&tbs, &fns) {
		(None, None) => {
			return Err(GqlError::NotConfigured);
		}
		(None, Some(fns)) if fns.len() == 0 => {
			return Err(GqlError::SchemaError("No Items Configured for Database".to_string()))
		}
		_ => {}
	}
	...
	trace!("current Query object for schema after tables: {:?}", query);
...
>>>>>>> main:crates/core/src/gql/schema.rs
	...
	trace!("current Query object for schema after functions: {:?}", query);
