...
pub fn kind_to_type(kind: Kind, types: &mut Vec<Type>) -> Result<TypeRef, GqlError> {
	...
	let out_ty = match match_kind {
		...
		Kind::Refs(ft, _) | Kind::DynRefs(ft, _) => {
...
pub fn gql_to_sql_kind(val: &GqlValue, kind: Kind) -> Result<SqlValue, GqlError> {
	...
	match kind {
		...
		Kind::Refs(_, _) => Err(resolver_error("Cannot convert value into refs")),
		Kind::DynRefs(_, _) => Err(resolver_error("Cannot convert value into dynrefs")),
	}
}
