...
impl TryFrom<ValueProto> for crate::expr::Value {
	...
	fn try_from(proto: ValueProto) -> Result<Self, Self::Error> {
		...
		let value = match inner {
			...
			ValueInner::Range(_v) => todo!("Range conversion not implemented"),
		};
...
impl TryFrom<RecordIdKeyProto> for crate::expr::Id {
	type Error = anyhow::Error;
...
impl TryFrom<crate::expr::Id> for RecordIdKeyProto {
	type Error = anyhow::Error;

	fn try_from(id: crate::expr::Id) -> Result<Self, Self::Error> {
		let inner = match id {
			crate::expr::Id::Number(v) => record_id_key_proto::Id::Int64(v),
