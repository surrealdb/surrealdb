	...
	pub fn nanos((val,): (i64,)) -> Result<Value, Error> {
		...
		match DateTime::from_timestamp(seconds, nanoseconds) {
			...
			None => Err(Error::InvalidArguments {
				...
				message: String::from("The argument must be an in-bounds number of nanoseconds relative to January 1, 1970 0:00:00 UTC that produces a datetime between between d'-262143-01-01T00:00:00Z' and d'+262142-12-31T23:59:59Z'."),
			}),
		}
	}
	...
	pub fn micros((val,): (i64,)) -> Result<Value, Error> {
		match DateTime::from_timestamp_micros(val) {
			...
			None => Err(Error::InvalidArguments {
				...
				message: String::from("The first argument must be an in-bounds number of microseconds relative to January 1, 1970 0:00:00 UTC that produces a datetime between between d'-262143-01-01T00:00:00Z' and d'+262142-12-31T23:59:59Z'."),
			}),
		}
	}
	...
	pub fn millis((val,): (i64,)) -> Result<Value, Error> {
		match DateTime::from_timestamp_millis(val) {
			...
			None => Err(Error::InvalidArguments {
				...
				message: String::from("The first argument must be an in-bounds number of milliseconds relative to January 1, 1970 0:00:00 UTC that produces a datetime between between d'-262143-01-01T00:00:00Z' and d'+262142-12-31T23:59:59Z'."),
			}),
		}
	}
	...
	pub fn secs((val,): (i64,)) -> Result<Value, Error> {
		match DateTime::from_timestamp(val, 0) {
			...
			None => Err(Error::InvalidArguments {
				...
				message: String::from("The first argument must be an in-bounds number of seconds relative to January 1, 1970 0:00:00 UTC that produces a datetime between between d'-262143-01-01T00:00:00Z' and d'+262142-12-31T23:59:59Z'."),
			}),
		}
	}
