...
pub mod cbor {
	...
	pub fn encode((arg,): (Value,)) -> Result<Value> {
		let val: CborValue = arg.try_into().map_err(|_| Error::InvalidArguments {
	...
	pub fn decode((arg,): (Bytes,)) -> Result<Value> {
		let cbor = ciborium::from_reader::<CborValue, _>(&mut arg.as_slice()).map_err(|_| {
			Error::InvalidArguments {
		...
		Value::try_from(cbor)
			.map_err(|v| Error::InvalidArguments {
				name: "encoding::cbor::decode".to_owned(),
				message: v.to_string(),
			})
