...
// (id, Optional(ft), Optional(ff)): (RecordId, Optional<String>,
// Optional<String>), ) -> Result<Value> {
// ensure!(
// ctx.get_capabilities().allows_experimental(&
// ExperimentalTarget::RecordReferences), Error::InvalidFunction {
...
// }
//
// async fn correct_refs_field(ctx: &Context, opt: &Options, ft: &Table, ff:
// Idiom) -> Result<Idiom> { Obtain the field definition
// let (ns, db) = opt.ns_db()?;
