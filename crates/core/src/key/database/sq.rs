...
pub(crate) struct SequenceDefinitionKey<'a> {
	__: u8,
	_a: u8,
	pub ns: NamespaceId,
	_b: u8,
...
impl_kv_key_storekey!(SequenceDefinitionKey<'_> => SequenceDefinition);
...
impl Categorise for SequenceDefinitionKey<'_> {
	fn categorise(&self) -> Category {
		Category::DatabaseSequence
	}
}

impl<'a> SequenceDefinitionKey<'a> {
...
mod tests {
	...
	fn key() {
		#[rustfmt::skip]
            let val = SequenceDefinitionKey::new(
            NamespaceId(1),
            DatabaseId(2),
            "test",
        );
		let enc = SequenceDefinitionKey::encode_key(&val).unwrap();
