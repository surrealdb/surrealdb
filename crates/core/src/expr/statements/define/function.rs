...
use crate::expr::function::FunctionVersion;
...
impl DefineFunctionStatement {
	...
	pub(crate) async fn args(&self) -> Result<&Vec<(Ident, Kind)>> {
		match &self.executable {
			Executable::Block {
				args,
				..
			} => Ok(args),
			_ => fail!("TODO compute args"),
		}
	}

	pub(crate) async fn returns(&self) -> Result<Option<&Kind>> {
		match &self.executable {
			Executable::Block {
				returns,
				..
			} => Ok(returns.as_ref()),
			_ => fail!("TODO compute returns"),
		}
	}
