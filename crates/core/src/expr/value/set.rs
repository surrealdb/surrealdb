...
mod tests {
	...
	async fn set_empty() {
		...
		let res: Value = Value::parse("{ test: 999 }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_blank() {
		...
		let mut val = Value::None;
		let res: Value = Value::parse("{ test: { something: 999 } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_basic() {
		...
		let idi: Idiom = SqlIdiom::parse("test.something").into();
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 } }");
		let res: Value = Value::parse("{ test: { other: null, something: 999 } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_allow() {
		...
		let mut val: Value = Value::parse("{ test: { other: null } }");
		let res: Value = Value::parse("{ test: { other: null, something: { allow: 999 } } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_other() {
		...
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 } }");
		let res: Value = Value::parse("{ test: { other: { something: 999 }, something: 123 } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_array() {
		...
		let res: Value = Value::parse("{ test: { something: [123, 999, 789] } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_array_field() {
		...
		let mut val: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 21 }] } }");
		let mut stack = reblessive::TreeStack::new();
	...
	async fn set_array_fields() {
		...
		let mut val: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res: Value = Value::parse("{ test: { something: [{ age: 21 }, { age: 21 }] } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(21))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_array_where_fields() {
		...
		let mut val: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res: Value = Value::parse("{ test: { something: [{ age: 34 }, 21] } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(21))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_array_where_fields_array_index() {
		...
		let mut val: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res: Value = Value::parse("{ test: { something: [21, { age: 36 }] } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(21))).finish().await.unwrap();
		assert_eq!(res, val);
	}
	...
	async fn set_array_where_fields_array_index_field() {
		...
		let mut val: Value = Value::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res: Value = Value::parse("{ test: { something: [{ age: 21 }, { age: 36 }] } }");
		let mut stack = reblessive::TreeStack::new();
		stack.enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(21))).finish().await.unwrap();
		assert_eq!(res, val);
	}
