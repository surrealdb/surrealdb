...
mod tests {
	...
	async fn patch_add_simple() {
		...
		let ops: Value = Value::parse("[{ op: 'add', path: '/temp', value: true }]");
		let res: Value = Value::parse("{ test: { other: null, something: 123 }, temp: true }");
	...
	async fn patch_remove_simple() {
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 }, temp: true }");
		let ops: Value = Value::parse("[{ op: 'remove', path: '/temp' }]");
	...
	async fn patch_replace_simple() {
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 }, temp: true }");
		let ops: Value = Value::parse("[{ op: 'replace', path: '/temp', value: 'text' }]");
		let res: Value = Value::parse("{ test: { other: null, something: 123 }, temp: 'text' }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_change_simple() {
		let mut val: Value =
			Value::parse("{ test: { other: null, something: 123 }, temp: 'test' }");
	...
	async fn patch_copy_simple() {
		let mut val: Value = Value::parse("{ test: 123, temp: true }");
		let ops: Value = Value::parse("[{ op: 'copy', path: '/temp', from: '/test' }]");
		let res: Value = Value::parse("{ test: 123, temp: 123 }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_move_simple() {
		let mut val: Value = Value::parse("{ temp: true, some: 123 }");
		let ops: Value = Value::parse("[{ op: 'move', path: '/other', from: '/temp' }]");
		let res: Value = Value::parse("{ other: true, some: 123 }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_test_simple() {
		let mut val: Value =
			Value::parse("{ test: { other: 'test', something: 123 }, temp: true }");
		let ops: Value = SqlValue::parse(
			"[{ op: 'remove', path: '/test/something' }, { op: 'test', path: '/temp', value: true }]",
		)
		.into();
		let res: Value = Value::parse("{ test: { other: 'test' }, temp: true }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_add_embedded() {
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 } }");
		let ops: Value = Value::parse("[{ op: 'add', path: '/temp/test', value: true }]");
		let res: Value =
	...
	async fn patch_remove_embedded() {
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 }, temp: true }");
		let ops: Value = Value::parse("[{ op: 'remove', path: '/test/other' }]");
		let res: Value = Value::parse("{ test: { something: 123 }, temp: true }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_replace_embedded() {
		let mut val: Value = Value::parse("{ test: { other: null, something: 123 }, temp: true }");
		let ops: Value = Value::parse("[{ op: 'replace', path: '/test/other', value: 'text' }]");
		let res: Value = Value::parse("{ test: { other: 'text', something: 123 }, temp: true }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_copy_embedded() {
		let mut val: Value = Value::parse("{ test: { other: null }, temp: 123 }");
		let ops: Value = Value::parse("[{ op: 'copy', path: '/test/other', from: '/temp' }]");
		let res: Value = Value::parse("{ test: { other: 123 }, temp: 123 }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_move_embedded() {
		let mut val: Value = Value::parse("{ test: { other: ':3', some: 123 }}");
		let ops: Value = Value::parse("[{ op: 'move', path: '/temp', from: '/test/other' }]");
		let res: Value = Value::parse("{ test: { some: 123 }, temp: ':3' }");
		val.patch(ops).unwrap();
		assert_eq!(res, val);
	}
	...
	async fn patch_change_invalid() {
		...
		let mut val: Value =
			Value::parse("{ test: { other: 'test', something: 123 }, temp: true }");
		let ops: Value = Value::parse("[{ op: 'change', path: '/test/other', value: 'text' }]");
