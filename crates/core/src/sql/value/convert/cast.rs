...
use crate::sql::{
	...
	Duration, File, Geometry, Ident, Kind, Literal, Number, Object, Range, Regex, SqlValue, Strand,
	Table, Thing, Uuid,
};
...
impl Cast for i64 {
	fn can_cast(v: &SqlValue) -> bool {
		match v {
			...
			SqlValue::Strand(ref v) => v.parse::<i64>().is_ok(),
			_ => false,
		}
	}
...
impl Cast for f64 {
	fn can_cast(v: &SqlValue) -> bool {
		match v {
			...
			SqlValue::Number(Number::Decimal(v)) => v.is_integer() || i64::try_from(*v).is_ok(),
			SqlValue::Strand(ref v) => v.parse::<f64>().is_ok(),
			_ => false,
		}
	}
