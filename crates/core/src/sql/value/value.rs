...
impl SqlValue {
	...
	pub fn ok(self) -> Result<SqlValue, Error> {
		Ok(self)
	}
	...
	pub(crate) fn writeable(&self) -> bool {
		match self {
			SqlValue::Cast(v) => v.writeable(),
			SqlValue::Block(v) => v.writeable(),
			SqlValue::Idiom(v) => v.writeable(),
			SqlValue::Array(v) => v.iter().any(SqlValue::writeable),
			SqlValue::Object(v) => v.iter().any(|(_, v)| v.writeable()),
			SqlValue::Function(v) => v.writeable(),
			SqlValue::Model(m) => m.args.iter().any(SqlValue::writeable),
			SqlValue::Subquery(v) => v.writeable(),
			SqlValue::Expression(v) => v.writeable(),
			_ => false,
		}
	}
}

crate::sql::impl_display_from_sql!(SqlValue);

impl crate::sql::DisplaySql for SqlValue {
...
impl TryPow for SqlValue {
	type Output = Self;
	fn try_pow(self, other: Self) -> Result<Self, Error> {
		Ok(match (self, other) {
			(SqlValue::Number(v), SqlValue::Number(w)) => Self::Number(v.try_pow(w)?),
...
mod tests {
	...
	fn check_size() {
		...
		assert!(104 >= std::mem::size_of::<Result<SqlValue, Error>>());
