...
impl SqlValue {
	...
	pub fn put(&mut self, path: &[Part], val: SqlValue) {
		match path.first() {
...
mod tests {
	...
	async fn put_none() {
		...
		let res = SqlValue::parse("999");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_empty() {
		...
		let mut val = SqlValue::None;
		let res = SqlValue::parse("{ test: 999 }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_blank() {
		...
		let mut val = SqlValue::None;
		let res = SqlValue::parse("{ test: { something: 999 } }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_basic() {
		let idi = Idiom::parse("test.something");
		let mut val = SqlValue::parse("{ test: { other: null, something: 123 } }");
		let res = SqlValue::parse("{ test: { other: null, something: 999 } }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_allow() {
		...
		let mut val = SqlValue::parse("{ test: { other: null } }");
		let res = SqlValue::parse("{ test: { other: null, something: { allow: 999 } } }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_other() {
		...
		let mut val = SqlValue::parse("{ test: { other: null, something: 123 } }");
		let res = SqlValue::parse("{ test: { other: { something: 999 }, something: 123 } }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_array() {
		...
		let res = SqlValue::parse("{ test: { something: [123, 999, 789] } }");
		val.put(&idi, SqlValue::from(999));
		assert_eq!(res, val);
	}
	...
	async fn put_array_field() {
		...
		let mut val = SqlValue::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res = SqlValue::parse("{ test: { something: [{ age: 34 }, { age: 21 }] } }");
		val.put(&idi, SqlValue::from(21));
		assert_eq!(res, val);
	}
	...
	async fn put_array_fields() {
		...
		let mut val = SqlValue::parse("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
		let res = SqlValue::parse("{ test: { something: [{ age: 21 }, { age: 21 }] } }");
		val.put(&idi, SqlValue::from(21));
		assert_eq!(res, val);
	}
