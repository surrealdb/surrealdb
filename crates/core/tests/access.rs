...
// 		res.remove(0).result.unwrap();
// 		//
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let ok =
// 			Regex::new(r"\{ ac: 'srv', creation: .*, expiration: .*, grant: \{ .* \}, id: .*, revocation:
// NONE, subject: \{ user: 'tobie' \}, type: 'bearer' \}") 					.unwrap();
...
// 			let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 			let ok =
// 				Regex::new(r"\{ ac: 'api', creation: .*, expiration: .*, grant: \{ .* \}, id: .*, revocation:
// NONE, subject: \{ record: user:tobie \}, type: 'bearer' \}") 						.unwrap();
// 			assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
// 			//
// 			let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 			let ok =
...
// 			let ok =
// 				Regex::new(r"\{ ac: 'api', creation: .*, expiration: .*, grant: \{ .* \}, id: .*, revocation:
// NONE, subject: \{ record: user:jaime \}, type: 'bearer' \}") 						.unwrap();
// 			assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
// 			//
// 			let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 			let ok =
...
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let re =
// 			Regex::new(r"\{ ac: 'srv', creation: .*?, expiration: .*, grant: \{ id: '(.*?)', key: .*? \},
// id: .*?, revocation: NONE, subject: \{ user: 'tobie' \}, type: 'bearer' \}") 					.unwrap();
...
// 			let ok =
// 				Regex::new(r"\{ ac: 'api', creation: .*, expiration: .*, grant: \{ .* \}, id: .*, revocation:
// d'.*?', subject: \{ record: user:tobie \}, type: 'bearer' \}") 						.unwrap();
// 			assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
// 			//
// 			let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 			let ok =
// 				Regex::new(r"\{ ac: 'api', creation: .*, expiration: .*, grant: \{ .* \}, id: .*, revocation:
// d'.*?', subject: \{ record: user:jaime \}, type: 'bearer' \}") 						.unwrap();
// 			assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
// 		}
// 	}
// }
...
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let ok = Regex::new(&format!(
// 			r"\[\{{ ac: 'srv', .*?, grant: \{{ id: '{kid}', key: '\[REDACTED\]' \}}, id: '{kid}',
// revocation: d'.*?', subject: \{{ user: 'tobie' \}}, type: 'bearer' \}}\]", 		))
// 		.unwrap();
// 		assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
...
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let ok = Regex::new(
// 			r"\[\{ ac: 'srv', .*?, revocation: NONE, subject: \{ user: 'tobie' \}, type: 'bearer' \}, \{
// ac: 'srv', .*?, revocation: NONE, subject: \{ user: 'tobie' \}, type: 'bearer' \}\]", 		)
// 		.unwrap();
// 		assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
...
// 		let res = &mut dbs
// 			.execute(
// 				"ACCESS srv SHOW WHERE revocation IS NONE AND (expiration IS NONE OR expiration >
// time::now())", 				&ses,
// 				None,
// 			)
// 			.await
...
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let re =
// 			Regex::new(r"\{ ac: 'srv', creation: .*?, expiration: d'.*?', grant: \{ id: '(.*?)', key: .*?
// \}, id: .*?, revocation: NONE, subject: \{ user: 'tobie' \}, type: 'bearer' \}") 					.unwrap();
// 		let kid = &re.captures(&tmp).unwrap()[1];
// 		// Consume the results of the other three
// 		res.remove(0).result.unwrap();
...
// 		let tmp = res.remove(0).result.unwrap().into_string().unwrap();
// 		let ok = Regex::new(r"\[\{ ac: 'srv', .*? \}, \{ ac: 'srv', .*? \}, \{ ac: 'srv', .*? \}, \{
// ac: 'srv', .*? \}, \{ ac: 'srv', .*? \}\]") 		.unwrap();
// 		assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
