...
use surrealml_llms::interface::run_model::run_model;
use surrealml_llms::models::model_spec::model_spec_trait::ModelSpec;
use surrealml_llms::models::model_spec::models::gemma::Gemma;
...
impl InvocationContext for DemoHost {
	...
	async fn ml_invoke_model(
		...
		Ok(run_model(&mut wrapper, input, 20).context("run_model should succeed")?.into_value())
	}
				...
				Ok(x) => {
					if let surrealdb_types::Value::Array(x) = x {
						let arr = x
							.into_iter()
							.map(|x| -> Result<f64> {
								if let surrealdb_types::Value::Number(
									surrealdb_types::Number::Float(x),
								) = x
								{
									Ok(x)
								} else {
