...
use surrealism_types::{
    arg::SerializableArg, args::Args, transfer::Transfer
};
...
pub mod kv {
    ...
    /// - If the FFI call or result reception encounters an issue.
    /// - If deserializing any result into `T` fails.
    pub fn entries<R: RangeBounds<String>, T: SurrealValue>(
        range: R,
    ) -> Result<Vec<(String, T)>> {
        let mut controller = Controller {};
        let range = SerializableRange::from_range_bounds(range)?.transfer(&mut controller)?;
...
pub mod ml {
        ...
        R: SurrealValue,
    {
        ...
        let input = SerializableArg::from(input).transfer(&mut controller)?;
        let weight = SerializableArg::from(weight).transfer(&mut controller)?;
        let weight_dir = SerializableArg::from(weight_dir.into()).transfer(&mut controller)?;
    ...
    {
        let mut controller = Controller {};
        let tokenizer = SerializableArg::from(tokenizer.into()).transfer(&mut controller)?;
        let input = SerializableArg::from(input).transfer(&mut controller)?;
