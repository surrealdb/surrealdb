...
///
/// # Type Parameters
/// - `A`: The argument type, which must implement `Args` for argument processing,
///   `Debug` for debugging, and be `'static + Send + Sync` for thread-safety and lifetime.
...
/// - `F`: The function type, a closure or function pointer that takes `A` and returns `R`,
///   also requiring `'static + Send + Sync`.
///
/// # Fields
