...
use surrealdb_types::{ToFlatbuffers, FromFlatbuffers, SurrealValue};
use anyhow::Result;
...
impl Serializable for String {
    ...
    fn deserialize(serialized: Serialized) -> Result<Self> {
        String::from_utf8(serialized.0.to_vec()).map_err(|e| anyhow::anyhow!("Invalid UTF-8 string: {}", e))
    }
}
...
impl<T: Serializable> Serializable for anyhow::Result<T> {
    ...
    fn deserialize(serialized: Serialized) -> Result<Self> {
        Ok(
            Result::<T, String>::deserialize(serialized)?
...
impl<T: Serializable> Serializable for SerializableRange<T> {
    ...
    fn deserialize(serialized: Serialized) -> Result<Self> {
        ...
        let range = SerializableRange { beg, end };
        Ok(range)
    }
}
