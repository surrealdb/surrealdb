...
impl SurrealismPackage {
    ...
    pub fn from_reader<R: Read>(reader: R) -> Result<Self> {
        ...
        for entry in archive
            .entries()
            .prefix_err(|| "Failed to read archive entries")?
        {
    ...
    pub fn pack(&self, output: PathBuf) -> Result<()> {
        ...
        let config_str = self
            .config
            .to_string()
            .prefix_err(|| "Failed to serialize config")?;
        ...
        let encoder = archive
            .into_inner()
            .prefix_err(|| "Failed to get encoder from archive")?;
        encoder
            .finish()
            .prefix_err(|| "Failed to finish zstd encoder")?;
