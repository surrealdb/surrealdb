use surrealdb_types::{SurrealValue, Value, object, array, Number};
...
fn test_enum_tagged_with_tag_and_content() {
	...
	let kind_foo = r#"Literal(Object({"content": Literal(Object({})), "tag": Literal(String("Foo"))}))"#;
	...
	assert_eq!(format!("{:?}", enum_kind), format!("Either([{kind_foo}, {kind_bar}, {kind_baz}, {kind_qux}])"));
...
fn test_enum_tagged_with_tag_variant_baz() {
	...
	let value = enum_tagged_variant.into_value();
	assert_eq!(value, Value::Object(object! { tag: Value::String("Baz".to_string()), content: Value::String("baz".to_string()) }));
...
fn test_enum_tagged_with_tag_variant_qux() {
	...
	let value = enum_tagged_variant.into_value();
	assert_eq!(value, Value::Object(object! { tag: Value::String("Qux".to_string()), content: Value::Array(array![Value::String("qux".to_string()), Value::Number(Number::Int(1))]) }));
...
fn test_enum_tagged_with_tag_variant_lowercase() {
	...
	let value = enum_tagged_variant.into_value();
	assert_eq!(value, Value::Object(object! { tag: Value::String("foo".to_string()), content: Value::Object(object! {}) }));
	...
	assert_eq!(format!("{:?}", enum_kind), r#"Literal(Object({"content": Literal(Object({})), "tag": Literal(String("foo"))}))"#);
