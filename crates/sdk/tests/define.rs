...
async fn define_statement_analyzer() -> Result<()> {
	...
	t.expect_val(
		r#"{
			...
			configs: {},
			functions: {
				"fn::stripHtml": "DEFINE FUNCTION fn::stripHtml($html: string) { RETURN string::replace($html, /<[^>]*>/, ''); } PERMISSIONS FULL"
			},
			models: {},
			params: {},
			tables: {},
...
async fn permissions_checks_define_function() {
	...
	let check_results = [
		vec![
			"{ accesses: {  }, analyzers: {  }, apis: {  }, buckets: {  }, configs: {  }, functions: { \"fn::greet\": \"DEFINE FUNCTION fn::greet() { RETURN 'Hello'; } PERMISSIONS FULL\" }, models: {  }, params: {  }, sequences: { }, tables: {  }, users: {  } }",
		],
		vec![
