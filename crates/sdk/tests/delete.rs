	...
	for ((level, role), (ns, db), should_succeed, msg) in tests.into_iter() {
...

		{
			...
			let mut resp = ds
				...
				.await
				.unwrap();
			let res = resp.remove(0).values.unwrap();
			...
			assert_eq!(res, ValueProto::none(), "delete should not fail");
...
async fn delete_filtered_live_notification() -> Result<()> {
	...
	assert_eq!(res.len(), 1);
	let live_id = res.remove(0).take_first().unwrap();
	...
	assert_eq!(
		...
		Notification::new(
			live_id,
			Action::Delete,
			Some(Thing::from(("person", "test_true"))),
			SqlValue::parse(
				"{
					id: person:test_true,
					condition: true,
