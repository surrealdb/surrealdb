...
async fn access_bearer_operations() {
	...
	for level in &test_levels {
		...
		let tmp = res.remove(0).take_first().unwrap().to_string();
		let ok =
			Regex::new(&format!(
		...
		assert!(ok.is_match(&tmp), "Output '{}' doesn't match regex '{}'", tmp, ok);
		//
		let values = res.remove(0).values.unwrap();
		assert_eq!(values.len(), 4);
		for value in values {
			let tmp = value.to_string();
			let ok = Regex::new(
				r"\{ ac: 'api', .*, grant: \{ id: .*, key: '\[REDACTED\]' \}, .* \}",
			)
			.unwrap();
			assert!(ok.is_match(&tmp), "Output '{tmp}' doesn't match regex '{ok}'");
		}
		//
		let values = res.remove(0).values.unwrap();
		assert_eq!(values.len(), 4);
...
async fn access_bearer_revoke() {
	...
	for level in &test_levels {
		...
		let res = &mut dbs
			...
			.await
			.unwrap();
		let tmp = Array::new(res.remove(0).values.unwrap()).to_string();
