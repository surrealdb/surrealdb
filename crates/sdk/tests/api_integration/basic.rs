...
use crate::api_integration::{ApiRecordId, Record as ARecord, RecordBuf};
...
pub async fn yuse(new_db: impl CreateDb) {
	...
	let err = db.create::<_, Option<Value>>(item.as_str()).await.unwrap_err();
	...
	db.create::<_, Option<Value>>(item).await.unwrap();
	drop(permit);
}
...
pub async fn invalidate(new_db: impl CreateDb) {
	...
	let error = db.create::<_, Option<ApiRecordId>>(("user", "john")).await.unwrap_err();
	assert!(
...
pub async fn query_with_stats(new_db: impl CreateDb) {
	...
	assert!(stats.execution_duration.unwrap().nanos > 0);
...
pub async fn create_record_no_id(new_db: impl CreateDb) {
	...
	let _: Value = db.create("user").await.unwrap();
}
...
pub async fn create_record_with_id(new_db: impl CreateDb) {
	...
	let _: Value = db.create(("user", "john")).await.unwrap();
	let _: Value = db.create("user:doe".parse::<RecordId>().unwrap()).await.unwrap();
}
...
pub async fn create_record_with_id_with_content(new_db: impl CreateDb) {
	...
	let value: Value = db
		.create("user:jane".parse::<RecordId>().unwrap())
		.content(ARecord {
	...
	assert_eq!(
		value.record(),
		Some("user:jane".parse::<RecordId>().unwrap())
	);
}
...
pub async fn create_record_with_id_in_content(new_db: impl CreateDb) {
	...
	// impl TryFromValue for Person {
	// 	fn try_from_value(value: ValueProto) -> Result<Self, anyhow::Error> {
	// 		let id = u32::try_from_value(value.remove("id").ok_or(anyhow::anyhow!("id not found"))?)?;
	// 		let name = String::try_from_value(value.remove("name").ok_or(anyhow::anyhow!("name not found"))?)?;
	// 		let job = String::try_from_value(value.remove("job").ok_or(anyhow::anyhow!("job not found"))?)?;
	// 		Ok(Person { id, name, job })
	// 	}
	// }

	impl TryFrom<Person> for Value {
		...
		fn try_from(value: Person) -> Result<Self, Self::Error> {
			...
			obj.insert("job".to_string(), value.job.into());
			Ok(Value::Object(Object::new(obj)))
		}
	}

	#[derive(Debug, Serialize, Deserialize)]
	pub struct ARecordWithId {
		#[allow(dead_code)]
		pub id: RecordId,
	}

	impl TryFromValue for ARecordWithId {
	...
	let error = db
		.create::<_, Option<RecordBuf>>(("user", "john"))
		.content(RecordBuf {
			id: ("user", "jane").into(),
			name: "John Doe".to_owned(),
		})
...
pub async fn insert_table(new_db: impl CreateDb) {
	...
	let _: Vec<ApiRecordId> = db.insert(table).content(json!([{ "foo": "bar" }])).await.unwrap();
	let _: Value = db.insert(table).await.unwrap();
	let _: Value = db.insert(table).content(json!({ "foo": "bar" })).await.unwrap();
	let _: Value =
		db.insert(table).content(json!([{ "foo": "bar" }])).await.unwrap();
...
pub async fn insert_thing(new_db: impl CreateDb) {
	...
	let _: Value = db.insert((table, "user3")).await.unwrap();
	let _: Value =
		db.insert((table, "user4")).content(json!({ "foo": "bar" })).await.unwrap();
...
pub async fn select_table(new_db: impl CreateDb) {
	...
	let _: Option<ApiRecordId> = db.create(table).await.unwrap();
	let _: Value = db.create(table).await.unwrap();
...
pub async fn select_record_id(new_db: impl CreateDb) {
	...
	let value: Value = db.select(RecordId::from(record_id)).await.unwrap();
	assert_eq!(value.record(), Some("user:john".parse::<RecordId>().unwrap()));
}
...
pub async fn select_record_ranges(new_db: impl CreateDb) {
	...
	let _: Value = db.create(RecordId::from((table, "zoey"))).await.unwrap();
	let convert = |users: Vec<ApiRecordId>| -> Vec<String> {
		users
			.into_iter()
			.map(|user| {
				let Id::String(ref x) = user.id.id else {
					panic!()
				};
				x.clone()
			})
	...
	let v: Value = db.select(table).range("jane"..="john").await.unwrap();
...
pub async fn select_records_fetch(new_db: impl CreateDb) {
	...
	let check_fetch = |mut response: QueryResults, expected: &str| {
		...
		let exp = expected.parse::<Value>().unwrap();
		assert_eq!(val, exp);
	};
...
pub async fn update_table(new_db: impl CreateDb) {
	...
	let _: Option<ApiRecordId> = db.create(table).await.unwrap();
	let _: Option<ApiRecordId> = db.create(table).await.unwrap();
	let _: Value = db.update(table).await.unwrap();
...
impl TryFromValue for Name {
	fn try_from_value(mut value: ValueProto) -> Result<Self, anyhow::Error> {
		let first = Cow::Owned(String::try_from_value(value.remove("first").ok_or(anyhow::anyhow!("first not found"))?)?);
		let last = Cow::Owned(String::try_from_value(value.remove("last").ok_or(anyhow::anyhow!("last not found"))?)?);
		Ok(Name { first, last })
	}
}
...
// 		let mut obj = BTreeMap::new();
// 		obj.insert("id".to_string(), value.id.into());
// 		obj.insert("title".to_string(), value.title.into());
// 		obj.insert("name".to_string(), value.name.into());
// 		obj.insert("marketing".to_string(), value.marketing.into());
// 		Ok(Value::Object(Object::new(obj)))
// 	}
// }
...
impl TryFromValue for Person {
	fn try_from_value(mut value: ValueProto) -> Result<Self, anyhow::Error> {
		let id = value.remove("id").map(RecordId::try_from_value).transpose()?;
		let title = Cow::Owned(String::try_from_value(value.remove("title").ok_or(anyhow::anyhow!("title not found"))?)?);
		let name = Name::try_from_value(value.remove("name").ok_or(anyhow::anyhow!("name not found"))?)?;
		let marketing = bool::try_from_value(value.remove("marketing").ok_or(anyhow::anyhow!("marketing not found"))?)?;
		Ok(Person { id, title, name, marketing })
	}
}
...
impl TryFromValue for Record {
	fn try_from_value(mut value: ValueProto) -> Result<Self, anyhow::Error> {
		let id = RecordId::try_from_value(value.remove("id").ok_or(anyhow::anyhow!("id not found"))?)?;
		let baz = String::try_from_value(value.remove("baz").ok_or(anyhow::anyhow!("baz not found"))?)?;
		let hello = Vec::<String>::try_from_value(value.remove("hello").ok_or(anyhow::anyhow!("hello not found"))?)?;
		Ok(Record { id, baz, hello })
	}
}
...
impl TryFrom<Record> for Value {
	...
	fn try_from(value: Record) -> Result<Self, Self::Error> {
		let mut obj = BTreeMap::new();
		obj.insert("id".to_string(), value.id.into());
		obj.insert("baz".to_string(), value.baz.into());
		obj.insert("hello".to_string(), value.hello.into());
		Ok(Value::Object(Object::new(obj)))
	}
}
...
pub async fn patch_record_id(new_db: impl CreateDb) {
	...
	let _: Option<Record> = db
		.update(("user", id))
		.patch(PatchOp::replace("/baz", "boo".into()))
		.patch(PatchOp::add("/hello", ["world"].into()))
...
pub async fn upsert_patch_record_id(new_db: impl CreateDb) {
	...
	let _: Option<Record> = db
		.update(("user", id))
		.patch(PatchOps::new().replace("/baz", "boo".into()).add("/hello", ["world"].into()).remove("/foo"))
		.await
		.unwrap();
	let value: Option<Record> = db.select(("user", id)).await.unwrap();
	assert_eq!(
...
pub async fn delete_record_id(new_db: impl CreateDb) {
	...
	assert!(jane.is_none());
	let value: Value = db.delete(RecordId::from(("user", "jane"))).await.unwrap();
...
pub async fn field_and_index_methods(new_db: impl CreateDb) {
	...
	let inside = as_value[0]["b1"]["total_peers"];

	assert_eq!(inside, &Value::Number(74.into()));
	assert!(!inside.is_none());
	assert_eq!(inside.into_option(), Some(&Value::Number(74.into())));

	let mut response =
		db.query("SELECT b1 FROM CREATE something SET b1.total_peers = 74").await.unwrap();
	let as_value: Value = response.take::<Value>(0).unwrap();
	let inside = as_value[0]["b1111111"]["total_peers"];
