...
use surrealdb_core::{
	cnf::PKG_VERSION,
	dbs::{Notification, QueryResult, Session, SessionId, SurrealDB, Variables},
	expr::Value,
	iam::{check::check_ns_db, Action, Auth, ResourceKind, Role},
	kvs::{export::Config, Datastore, LockType, TransactionType},
	vars,
};
...
impl SurrealDBGrpcService {
	...
	fn load_session<R>(
		...
		let role = Role::Owner;
		let default_session_for_testing = Session {
			au: Arc::new(Auth::for_root(role)),
			ac: Some("api".into()),
			rt: true,
		...
		Ok((*session_id, Arc::new(default_session_for_testing)))
	}
	...
	fn set_session(&self, session_id: SessionId, session: Arc<Session>) {
		...
		// 		variables: Variables::new(),
		// 	};
		// 	self.sessions.insert(session_id, ArcSwap::new(Arc::new(default_session_for_testing)));
		// };
...
impl rpc_proto::surreal_db_service_server::SurrealDbService for SurrealDBGrpcService {
	...
	async fn signin(
		...
		let mut session = session.as_ref().clone();
		// Attempt signin, mutating the session directly
	...
	async fn authenticate(
		...
		let mut session = session.as_ref().clone();
		// Attempt authentication, mutating the session directly
		let out: anyhow::Result<Value> =
	...
	async fn r#use(
		...
		// Clone the current session
		let mut session = session.as_ref().clone();
		// Update the selected namespace directly
	...
	async fn set(
		...
		// Compute the specified parameter directly
		let value = self
			.datastore()
	...
	async fn reset(
		...
		// Clone the current session
		let mut session = session.as_ref().clone();
		// Reset the current session directly
	...
	async fn import_sql(
		...
		// Process import statements directly
	...
	async fn export_sql(
		...
		let future = match datastore.export_with_config(&session, tx, config).await {
			...
			Err(err) => {
				...
				return Ok(tonic::Response::new(Box::pin(output_stream) as Self::ExportSqlStream));
			}
		};
	...
	async fn export_ml_model(
		...
		let kvs = self.datastore();
		// Process ML model export directly
		tokio::spawn(async move {
	...
	async fn query(
		...
		let datastore = self.datastore();
		// Execute query directly without spawning extra task
		let res = match datastore.execute(&query, &session, variables).await {
			...
			Err(err) => {
				if let Err(err) = tx.send(Err(tonic::Status::internal(err.to_string()))).await {
					error!("Failed to send query error to client: {err}");
				}
				let output_stream = ReceiverStream::new(rx);
				return Ok(tonic::Response::new(Box::pin(output_stream) as Self::QueryStream));
			}
		};

		// Process results directly without spawning
		tokio::spawn(async move {
			for (
				query_index,
				QueryResult {
			...
			) in res.into_iter().enumerate()
			{
				...
				if let Err(err) = tx.send(Ok(query_result_proto)).await {
	...
	async fn subscribe(
		...
		// Execute the query on the database directly
		let res = self
			.datastore()
