...
use surrealdb_types::{Value as SurValue, sql::ToSqon};
				...
				MismatchKind::Value(ref x) => f.indent(|f| {
					...
					match x {
						...
						ValueMismatchKind::InvalidValue {
							...
							f.indent(|f| {
								writeln!(f, "= Expected:")?;
								f.indent(|f| writeln!(f, "- Value: {}", expected.to_sqon()))?;
								writeln!(f, "= Got:")?;
								f.indent(|f| writeln!(f, "- Value: {}", got.to_sqon()))?;
								writeln!(f, "= Diff:")?;
								f.indent(|f| Self::display_diff(&got.to_sqon(), &expected.to_sqon(), use_color, f))
							})
						}
	...
	fn display_expectation(v: &TestValueExpectation, f: &mut Fmt) -> fmt::Result {
		match v {
			...
			TestValueExpectation::Value(v) => match v {
				Some(v) => writeln!(f, "- Value: {}", v.expected.to_sqon()),
