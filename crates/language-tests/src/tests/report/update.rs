	...
	if let Some(arr) = results_array.as_array_of_tables_mut() {
		...
		for (idx, r) in values.iter().enumerate() {
			if let Some(x) = arr.get_mut(idx) {
				match r {
					Ok(r) => {
						x["value"] = toml_edit::value(r.to_sqon());
					}
					Err(e) => {
				...
				match r {
					Ok(r) => {
						table["value"] = toml_edit::value(r.to_sqon());
					}
					Err(e) => {
		...
		if values.iter().any(|x| x.is_err()) {
			...
			for r in values.iter() {
				...
				match r {
					Ok(x) => {
						table.insert("value", toml_edit::value(x.to_sqon()));
					}
					Err(e) => {
			...
			for (idx, r) in values.iter().enumerate() {
				let v = toml_edit::value(r.as_ref().unwrap().to_sqon()).into_value().unwrap();
				if let Some(x) = arr.get_mut(idx) {
					*x = v;
				} else {
					arr.push(v)
