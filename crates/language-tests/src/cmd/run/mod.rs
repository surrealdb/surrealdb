...
pub async fn run(matches: &ArgMatches) -> Result<()> {
    ...
    let num_jobs = matches.get_one::<u32>("jobs").copied().unwrap_or_else(|| {
    ...
    for (id, test) in runner.set.iter_ids() {
        ...
        let future = async move {
            ...
            if let Err(e) = future.await {
                warn!(
                    "{:?}",
                    e.context(format!("Failed to run test '{name_clone}'"))
                )
            }
        };
    ...
    match failure_mode {
        ...
        FailureMode::Overwrite => {
            for x in reports
                .iter()
                .filter(|x| !x.succeeded() && !x.has_error() && !x.is_wip())
            {
                x.update_config_results(&runner.set).await?;
            }
        }
...
pub async fn run_test(id: TestId, runner: TestRunner, dbs: Option<Datastore>) -> Result<()> {
        ...
        T: std::cmp::Eq + std::hash::Hash + Clone,
    {
        v.as_ref()
            ...
            .unwrap_or(if deny { Targets::None } else { Targets::All })
    }
...
async fn run_test_with_dbs(
    ...
    if let Some(ref ns) = session.ns {
        if let Some(ref db) = session.db {
            ...
            dbs.execute(
                &format!("REMOVE DATABASE IF EXISTS `{db}`;"),
                &session,
                None,
            )
            .await
    ...
    if let Some(ref ns) = session.ns {
        ...
        dbs.execute(
            &format!("REMOVE NAMESPACE IF EXISTS `{ns}`;"),
            &session,
            None,
        )
        .await
