...
This table can either be a straight table or an array of tables. Possible
versions are:
```
[test.results]
parse-error = "foo"
...
specify the actual expected output of the tests. 

A single SurrealQL query can consist of multiple statements which produce either
zero or one result. The test allow specifying the expected number and value of
the results. Generally test will look like the following
...
error = true
```
Here we specify that the test should return two results. The first should be a
...
routs and scripting just like with the SurrealDB binary/rust SDK.
