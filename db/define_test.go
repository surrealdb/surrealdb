...
func TestDefine(t *testing.T) {
	...
	Convey("Define a single-field unique index on a table, and ensure it prevents duplicate record values, after deleting records", t, func() {
		...
		So(res[9].Result, ShouldHaveLength, 2)
		So(data.Consume(res[9].Result[0]).Get("id").Data(), ShouldResemble, &sql.Thing{"person", "tre"})
		So(data.Consume(res[9].Result[1]).Get("id").Data(), ShouldResemble, &sql.Thing{"person", "two"})
	...
	Convey("Define a multiple-field unique index on a table, and ensure it prevents duplicate record values, after updating records", t, func() {
	...
	Convey("Define a multiple-field unique index on a table, and ensure it prevents duplicate record values, after deleting records", t, func() {
