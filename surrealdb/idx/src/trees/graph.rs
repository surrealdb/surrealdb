	...
	S: DynamicSet,
{
	...
	pub(in crate::trees) fn nodes(&self) -> &HashMap<ElementId, S> {
		&self.nodes
	}
	pub(in crate::trees) fn check(&self, g: Vec<(ElementId, Vec<ElementId>)>) {
		for (n, e) in g {
...
#[cfg(test)]
mod tests {
	use crate::trees::dynamicset::{AHashSet, ArraySet, DynamicSet};
