//! Execution-time observability primitives for SurrealDB.
//!
//! This crate defines the [`ExecutionObserver`] trait and the event types it
//! receives, plus the [`ObservabilityProvider`] composer extension used by the
//! server to supply an observer at startup. It is a leaf crate: it depends only
//! on `surrealdb-types`, `surrealdb-rpc`, and `surrealdb-iam`, never on the core
//! engine. Core-side glue that has to read engine types (building event context
//! from a session, classifying `anyhow`/kvs errors, classifying statement
//! expressions) lives in `surrealdb-core`.
//!
//! # Data safety
//!
