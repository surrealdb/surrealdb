...
impl StatementType {
	...
	/// Returns `true` if this statement type is a data-mutation statement
	/// (CREATE / UPDATE / UPSERT / DELETE / RELATE / INSERT).
...
///
/// Built once per request from the active session (in `surrealdb-core`) and
/// stashed on the request context (and on each transaction derived from that
/// context) so emit sites can populate the `*Ctx` half of each event without
/// re-parsing the session value tree on every call.
///
/// `user` follows the standard record-access collapsing rule: anonymous
/// sessions map to `None`, record-access principals to a fixed `<record>`
/// sentinel, and everything else to the actor id.
...
pub struct StatementEventCtx {
	...
	/// via [`crate::observer::ExecutionObserver::needs_statement_text`]. Must
...
///
/// `method` holds the already-bounded [`surrealdb_rpc::Method`] and is safe to
/// emit as an attribute because all variants are known at compile time.
#[derive(Clone, Copy, Debug)]
pub struct RpcEventSafe {
	pub method: surrealdb_rpc::Method,
	pub outcome: Outcome,
	pub duration: Duration,
	/// Bounded error classification when `outcome` is [`Outcome::Error`].
	pub error_class: Option<&'static str>,
