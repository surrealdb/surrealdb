 ...
 * out of the Cargo workspace. That keeps the addon's version identical to the
 * `surrealdb` release whose engine it embeds, with nothing to bump by hand.
 *
 * Expects `dist/` to already hold a build for every platform being published
 * (`bun run build.ts` produces one platform's worth).
 */
