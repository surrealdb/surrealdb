	...
	fn call(&mut self, req: HttpRequest<B>) -> Self::Future {
		...
		Box::pin(async move {
			...
			if is_accept_multipart_mixed {
				...
				let mut req_with_data =
					graphql_req.into_inner().data(datastore_ctx).data(session_ctx);
				...
				if request_is_subscription(&mut req_with_data) {
					...
					// `serve_graphql_ws` (see `ntw::graphql`) and never
				...
				// to `"batch"`.
				let mut batch_req = graphql_req.into_inner();
