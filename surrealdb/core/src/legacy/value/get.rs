...
use crate::expr::idiom::recursion::Recursion;
...
use crate::expr::{ControlFlow, Error as ExprError, Expr, FlowResult, Idiom, Literal, Lookup};
use crate::fnc::idiom;
use crate::legacy::compute_idiom_recursion;
use crate::val::value::get::expr_references_parent;
...
pub(crate) async fn value_get(
	...
	match first {
		...
		Part::Recurse(recurse, inner_path, instruction) => {
			...
			if !after.is_empty() {
				stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, after.as_slice()))
					.await
			} else {
				Ok(v)
			}
		...
		Part::Doc => {
			...
			stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, path.next())).await
		}
		...
		p => match this {
			...
			Value::Geometry(v) => match p {
				...
				Part::Field(f) if f == "type" => {
					...
					stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, path.next()))
						.await
				}
				...
				Part::Destructure(_) => {
					...
					stk.run(|stk| crate::legacy::value_get(&obj, stk, ctx, opt, doc, path)).await
				}
				Part::Method(name, args) => {
					let a = stk
						.scope(|scope| {
							try_join_all(args.iter().map(|v| {
								scope.run(|stk| crate::legacy::expr_compute(v, stk, ctx, opt, doc))
							}))
						})
						.await?;
					let v =
						stk.run(|stk| idiom(stk, ctx, opt, doc, v.clone().into(), name, a)).await?;
					stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, path.next()))
						.await
				}
				Part::Optional => {
					stk.run(|stk| crate::legacy::value_get(this, stk, ctx, opt, doc, path.next()))
						.await
				}
			...
			Value::Object(v) => match p {
				Part::Lookup(_) => match v.rid() {
					Some(v) => {
						...
						stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, path)).await
					}
					None => {
						stk.run(|stk| {
							crate::legacy::value_get(&Value::None, stk, ctx, opt, doc, path.next())
						})
						.await
					}
				},
				...
				Part::Value(x) => match stk
					.run(|stk| crate::legacy::expr_compute(x, stk, ctx, opt, doc))
					.await
					.catch_return()?
				{
					Value::Number(n) => match v.get(&n.to_sql()) {
						Some(v) => {
							stk.run(|stk| {
								crate::legacy::value_get(v, stk, ctx, opt, doc, path.next())
							})
							.await
						}
						None => {
				...
				Part::Destructure(p) => {
					let cur_doc = CursorDoc::from(this.clone());
					...
					for p in p.iter() {
						...
						obj.insert(
							p.field(),
							stk.run(|stk| {
								crate::legacy::idiom_compute(&idiom, stk, ctx, opt, Some(&cur_doc))
							})
							.await?,
						);
					}
					...
					stk.run(|stk| crate::legacy::value_get(&obj, stk, ctx, opt, doc, path.next()))
						.await
				}
				Part::Method(name, args) => {
					let args = stk
					...
					let res = stk
						.run(|stk| idiom(stk, ctx, opt, doc, v.clone().into(), name, args.clone()))
						.await;

					let res = fallback_function! {
						if res => InvalidFunction(e) then {
							if let Some(Value::Closure(x)) = v.get(name) {
								crate::legacy::closure_invoke(x, stk, ctx, opt, doc, args).await
							} else {
								Err(e)
							}
						}
					...
					stk.run(|stk| crate::legacy::value_get(&res, stk, ctx, opt, doc, path.next()))
						.await
				}
				Part::Optional => {
					stk.run(|stk| crate::legacy::value_get(this, stk, ctx, opt, doc, path.next()))
			...
			Value::RecordId(v) => {
				...
				match p {
					...
					Part::Lookup(g) => {
						...
						let res = stk
							.run(|stk| {
								crate::legacy::select_statement_compute(&stm, stk, ctx, opt, doc)
							})
							.await?
							.all();
							...
							match res {
								...
								Value::Array(_) => {
									return Err(crate::expr::ControlFlow::Err(anyhow::anyhow!(
										crate::exec::Error::SingleOnlyOutput
									)));
								}
								other => other,
							}
						...
						if last_part {
							...
							let res = stk
								.run(|stk| {
									crate::legacy::value_get(&res, stk, ctx, opt, doc, path.next())
								})
								.await?;
					...
					Part::Method(name, args) => {
						let a = stk
							.scope(|scope| {
								try_join_all(args.iter().map(|v| {
									scope.run(|stk| {
										crate::legacy::expr_compute(v, stk, ctx, opt, doc)
									})
								}))
							})
							.await?;
						...
						let res = stk
							.run(|stk| idiom(stk, ctx, opt, doc, v.clone().into(), name, a.clone()))
							.await?;
					...
					Part::Optional => {
						stk.run(|stk| {
							crate::legacy::value_get(this, stk, ctx, opt, doc, path.next())
						})
						.await
					}
					...
					//     `self.initial` and `self.current`, so `store_index_data`'s `o != n` guard
					//     suppresses index maintenance and leaves stale entries (Codex finding
					//     9c442c96).
					...
					p => {
							...
							(Part::Value(x), RecordIdKey::Object(obj)) => {
									...
									.catch_return()?
								{
									Value::String(s) if obj.contains_key(s.as_str()) => {
										let v = obj.get(s.as_str()).cloned().unwrap_or(Value::None);
										return stk
											.run(|stk| {
												crate::legacy::value_get(
													&v,
									...
									x => &[&[Part::Value(x.into_literal())], path.next()].concat(),
								}
							}
							...
							&& cur.rid.as_deref() == Some(&val)
						{
							...
							return stk
								.run(|stk| {
									crate::legacy::value_get(&current, stk, ctx, opt, None, next)
								})
								.await;
						}

						// Fetch the record id's contents
						let v = crate::legacy::record_id_select_document(val, stk, ctx, opt, doc)
							.await?
							.map(Value::Object)
							.unwrap_or(Value::None);

						// Continue processing the path on the now fetched record
						stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, None, next)).await
					}
				}
			}
			v => {
				match p {
					Part::Optional => match &this {
					...
					Part::Flatten => {
						stk.run(|stk| crate::legacy::value_get(v, stk, ctx, opt, None, path.next()))
							.await
					}
					Part::Method(name, args) => {
						let a = stk
							...
							.await?;
						let v =
							stk.run(|stk| idiom(stk, ctx, opt, doc, v.clone(), name, a)).await?;
						stk.run(|stk| crate::legacy::value_get(&v, stk, ctx, opt, doc, path.next()))
							.await
					}
...
mod tests {
	...
	async fn get_none() {
		...
		let res = stack
			.enter(|stk| crate::legacy::value_get(&val, stk, &ctx, &opt, None, &idi))
			.finish()
			.await
			.unwrap();
		assert_eq!(res, val);
