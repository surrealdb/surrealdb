...
use crate::expr::Base;
use crate::expr::statements::info::{InfoStatement, InfoStructure, process};
...
pub(crate) async fn info_statement_compute(
	this: &InfoStatement,
	stk: &mut Stk,
	ctx: &FrozenContext,
	opt: &Options,
	...
	match this {
		InfoStatement::Root(structured, version) => {
			...
			let version = match version {
				Some(v) => Some(
					stk.run(|stk| crate::legacy::expr_compute(v, stk, ctx, opt, None))
						.await
						.catch_return()?
						.cast_to::<Datetime>()?
...
pub(crate) async fn get_module_exports(
	ctx: &FrozenContext,
...
pub(crate) async fn system() -> Value {
...
#[cfg(feature = "surrealism")]
pub(crate) fn surrealism_exports_manifest_to_value(
