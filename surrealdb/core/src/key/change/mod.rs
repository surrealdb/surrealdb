...
mod tests {
	...
	fn cf_key() {
		...
		let ts1 = ts_impl.from_versionstamp(12345).unwrap().to_ts_bytes();
		...
		let ts2 = ts_impl.from_versionstamp(12346).unwrap().to_ts_bytes();
	...
	fn ts_prefix_key() {
		let ts_impl = TimeStampImpl::Default;
		let ts = ts_impl.from_versionstamp(12345).unwrap().to_ts_bytes();
