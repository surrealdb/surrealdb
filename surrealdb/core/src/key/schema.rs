...
use crate::catalog::{DatabaseId, IndexId, NamespaceId, Record};
...
use crate::key::{
	KVKey, KVKeyDecode, KVRange, KVSubspace, KVValue, Key, KeyRange, RawRange, TypedRange,
};
use crate::kvs::index::{
...
use crate::val::{IndexFormat, RecordId, RecordIdKey, Value};
...
keyspace! {
	...
	root = ["/"] (format_generic) {
		...
		node = ["!nd", @, nd: Uuid] => crate::dbs::node::Node;

		/// A root-level user definition.
		root_user = ["!us", @, user: Str] => crate::catalog::UserDefinition;

		/// A root-level access method definition.
		root_access_method = ["!ac", @, ac: Str] => crate::catalog::StoredAccessDefinition;

		/// Root-level configuration, keyed by kind.
		root_config = ["!cg", @, ty: Str] => crate::catalog::StoredConfigDefinition;

		/// A lease over one of the singleton background tasks.
		task_lease = ["!tl", @, task_id: TaskLeaseType] => crate::catalog::TaskLease;

		/// A durable session.
		session = ["!se", @, id: Uuid] => crate::dbs::DurableSession;

		/// The namespace id allocator's reserved batch and its state.
		ns_id_batch = ["!nh", @, start: i64] => crate::kvs::sequences::BatchValue;
		ns_id_state = ["!ni", @, nid: Uuid] => crate::kvs::sequences::SequenceState;
		...
		event_queue = ["!eq", @, ns: NamespaceId, db: DatabaseId, tb: Table, ev: Str,
					   ts: u64, node_id: Uuid] => crate::doc::AsyncEventRecord;
		...
		/// beneath them do.
		node_state = ["$", nd: Uuid] {
			node_live_query = ["!lq", @, lq: Uuid] => crate::catalog::NodeLiveQuery;
		}

		/// Grants issued against a root-level access method.
		root_access = ["&", ac: Str] {
			root_grant = ["!gr", @, gr: Str] => crate::catalog::AccessGrant;
		}

		ns = ["*", ns: NamespaceId] (format_generic) {
			/// A database definition, keyed by name within its namespace.
			database = ["!db", @, db: Str] => crate::catalog::DatabaseDefinition;

			/// A namespace-level user definition.
			ns_user = ["!us", @, user: Str] => crate::catalog::UserDefinition;

			/// A namespace-level access method definition.
			ns_access_method = ["!ac", @, ac: Str] => crate::catalog::StoredAccessDefinition;

			/// The database id allocator's reserved batch and its state.
			db_id_batch = ["!dh", @, start: i64] => crate::kvs::sequences::BatchValue;
			db_id_state = ["!di", @, nid: Uuid] => crate::kvs::sequences::SequenceState;

			/// Grants issued against a namespace-level access method.
			ns_access = ["&", ac: Str] {
				ns_grant = ["!gr", @, gr: Str] => crate::catalog::AccessGrant;
			}

			db = ["*", db: DatabaseId] (format_generic) {
				/// A table definition, keyed by name within its database.
				table = ["!tb", @, tb: Table] => crate::catalog::StoredTableDefinition;

				/// A database-level user definition.
				db_user = ["!us", @, user: Str] => crate::catalog::UserDefinition;

				/// A database-level access method definition.
				db_access_method = ["!ac", @, ac: Str] => crate::catalog::StoredAccessDefinition;

				api = ["!ap", @, ap: Str] => crate::catalog::StoredApiDefinition;
				analyzer = ["!az", @, az: Str] => crate::catalog::AnalyzerDefinition;
				bucket = ["!bu", @, bu: Str] => crate::catalog::StoredBucketDefinition;
				db_config = ["!cg", @, ty: Str] => crate::catalog::StoredConfigDefinition;

				/// A user-defined function. The tag is `!fn`, not `!fc`.
				function = ["!fn", @, fc: Str] => crate::catalog::StoredFunctionDefinition;

				module = ["!md", @, md: Str] => crate::catalog::StoredModuleDefinition;

				/// A machine-learning model, keyed by name and version.
				ml_model = ["!ml", @, ml: Str, vn: Str] => crate::catalog::StoredMlModelDefinition;

				param = ["!pa", @, pa: Str] => crate::catalog::StoredParamDefinition;

				/// The table id allocator's reserved batch and its state.
				tb_id_batch = ["!th", @, start: i64] => crate::kvs::sequences::BatchValue;
				tb_id_state = ["!ti", @, nid: Uuid] => crate::kvs::sequences::SequenceState;
				...
				db_access = ["&", ac: Str] {
					db_grant = ["!gr", @, gr: Str] => crate::catalog::AccessGrant;
				}
				...
				tbl = ["*", tb: Table] (format_generic) {
					...
					event = ["!ev", @, ev: Str] => crate::catalog::StoredEventDefinition;
					field = ["!fd", @, fd: Str] => crate::catalog::StoredFieldDefinition;

					/// A table that computes its rows from this one.
					foreign_table = ["!ft", @, ft: Table]
						=> crate::catalog::StoredTableDefinition;

					/// An index definition, plus the reverse lookup from the index's
					/// id back to its name.
					index_name = ["!il", ix: IndexId] => String (derive(+Ord));
					index_def = ["!ix", @, ix: Str]
						=> crate::catalog::StoredIndexDefinition (derive(+Ord));

					/// A live-query subscription against this table.
					subscription = ["!lq", @, lq: Uuid]
						=> crate::catalog::StoredSubscriptionDefinition;

					/// The document id allocator's reserved batch and its state.
					doc_id_batch = ["!dh", @, start: i64] => crate::kvs::sequences::BatchValue;
					doc_id_state = ["!ds", @, nid: Uuid] => crate::kvs::sequences::SequenceState;
					...
					index_id_batch = ["!ih", @, start: i64] => crate::kvs::sequences::BatchValue;
					index_id_state = ["!is", @, nid: Uuid] => crate::kvs::sequences::SequenceState;
					...
					idx = ["+", ix: IndexId] (format_generic) {
						...
						entry = ["*", @, fd: [Value..], @open, @, raw 0x03, id: Id]
							=> crate::idx::entry::IndexEntryValue (derive(-Eq));
						...
						term_posting = term_docs + [id: DocId]
							=> crate::idx::ft::fulltext::TermDocument;
						...
						doc_stats = ["!dc"] => crate::idx::ft::fulltext::DocLengthAndCount
							(also_range);
						doc_stats_delta = doc_stats + [doc_id: DocId, nid: Uuid, uid: Uuid]
							=> crate::idx::ft::fulltext::DocLengthAndCount;

						doc_length = ["!dl", @, id: DocId] => crate::idx::ft::DocLength;
						doc_count = ["!dv"] => u64;
						...
						index_primary = ["!ip", id: RecordIdKey]
							=> crate::kvs::index::PrimaryAppending;

						/// HNSW graph storage.
						hnsw_state = ["!hs"] => crate::idx::trees::hnsw::HnswState;
						hnsw_generation = ["!hg"] => u64;
						hnsw_vector = ["!he", @, element_id: ElementId]
							=> crate::idx::trees::vector::SerializedVector;
						hnsw_element = ["!hv", @, vec: Vector]
							=> crate::idx::trees::hnsw::docs::ElementDocs (derive(-Eq, -PartialOrd));
						hnsw_element_hashed = ["!hh", @, hash: Hash32]
							=> crate::idx::trees::hnsw::docs::ElementHashedDocs;
						hnsw_layer = ["!hl", layer: u16, @, chunk: u32] => Vec<u8>;
						hnsw_node = ["!hn", layer: u16, @, node: ElementId] => Vec<u8>;

						#[format(IndexFormat)]
						hnsw_record_pending = ["!hr", @, id: Id]
							=> crate::idx::trees::hnsw::HnswRecordPendingUpdate;
						...
						#[cfg(diskann)]
						diskann_state = ["!ds"] => crate::idx::trees::diskann::DiskAnnState;
						#[cfg(diskann)]
						diskann_generation = ["!dg"] => u64;
						#[cfg(diskann)]
						diskann_element = ["!de", @, element_id: ElementId]
							=> crate::idx::trees::diskann::DiskAnnElement;
						#[cfg(diskann)]
						diskann_node = ["!dn", @, element_id: ElementId]
							=> crate::idx::trees::diskann::DiskAnnNode;
						#[cfg(diskann)]
						diskann_element_docs = ["!dq", @, vec: Vector]
							=> crate::idx::trees::diskann::docs::DiskAnnElementDocs
							(derive(-Eq, -PartialOrd));
						#[cfg(diskann)]
						diskann_element_hashed = ["!dh", @, hash: Hash32]
							=> crate::idx::trees::diskann::docs::DiskAnnElementHashedDocs;
						#[cfg(diskann)]
						diskann_pending_legacy = ["!dp", @, shard: u16]
							=> crate::idx::trees::diskann::DiskAnnPendingState;
						#[cfg(diskann)]
						...
						diskann_record_pending = ["!dr", @, id: Id]
							=> crate::idx::trees::diskann::DiskAnnRecordPendingUpdate;
						#[cfg(diskann)]
						#[format(IndexFormat)]
...
mod tests {
	...
	///
	/// Regenerate with `RESULT=OVERWRITE cargo test -p surrealdb-core keyspace_map`
