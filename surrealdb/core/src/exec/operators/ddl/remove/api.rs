...
impl ExecOperator for RemoveApiPlan {
	ddl_operator_common!("RemoveApi", ContextLevel::Database);
...
async fn execute(
	...
	let ap = match txn.get_db_api(ns, db, &name).await? {
		Some(x) => x,
		None => {
			if if_exists {
