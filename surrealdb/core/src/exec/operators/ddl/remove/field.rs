...
impl ExecOperator for RemoveFieldPlan {
	ddl_operator_common!("RemoveField", ContextLevel::Database);
...
async fn execute(
	...
	let Some(_fd) = txn.get_tb_field(ns, db, &table_name, &name).await? else {
		if if_exists {
