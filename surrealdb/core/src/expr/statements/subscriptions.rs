//! Telling live subscribers that the keys they wait on are being destroyed.
//!
//! A subscription lives at `/*{ns}*{db}*{tb}!lq{id}`, inside the table's key
//! range. Any statement that wipes that range takes every subscription on the
//! table with it, so each of those statements owes its subscribers the one
//! notification that tells them to stop waiting. These helpers live here, above
//! the individual statements, because the obligation follows the key range
//! rather than any one statement: `REMOVE TABLE`, `REMOVE DATABASE`,
//! `REMOVE NAMESPACE` and `DEFINE TABLE ... OVERWRITE ... AS SELECT` all reach
//! it.
