...
pub struct StoredFunctionDefinition {
	...
	pub(crate) old_args: Vec<(String, Kind)>,
	#[revision(start = 4)]
	pub(crate) args: Vec<(String, KindText)>,
	/// Canonical SurrealQL text of the function's `{ ... }` body block.
	pub(crate) block: BlockText,
	...
	pub(crate) old_returns: Option<Kind>,
	#[revision(start = 4)]
	pub(crate) returns: Option<KindText>,
...
impl FunctionDefinition {
	pub(crate) fn to_stored(&self) -> StoredFunctionDefinition {
