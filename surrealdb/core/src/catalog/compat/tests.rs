...
use super::super::*;
use super::{fixtures, v3_0_0, v3_0_0_beta_1, v3_0_0_beta_3, v3_1_0, v3_1_1, v3_2_0};
...
///
/// Historical snapshots (`v3_0_0_*`, `v3_1_0`, and now `v3_1_1`) were
/// captured under an earlier encoder; the current encoder produces
/// different bytes for some of their fixtures, so a byte-match comparison
/// would fail spuriously for them. They keep their decode-and-equals
/// assertion only.
///
...
///
/// `v3_1_1` was demoted from current-format when 3.2.0 added the
/// `ratelimits` field to `TableDefinition` (revision 3) and
/// `FieldDefinition`: the decoder still reads the older revisions, but
/// the encoder now writes the new revision, so re-encoding no longer
/// reproduces the frozen 3.1.1 bytes for those types.
///
...
const fn version_writes_current_format(version_name: &str) -> bool {
	...
	matches!(version_name.as_bytes(), b"v3_2_0")
}
...
compat_test!(
	...
	[v3_0_0_beta_1, v3_0_0_beta_3, v3_0_0, v3_1_0, v3_1_1, v3_2_0]
);
compat_test!(
...
compat_test!(
	table_with_ratelimit,
	TableDefinition,
	TABLE_WITH_RATELIMIT,
	fixtures::table_with_ratelimit(),
	[v3_2_0]
);
compat_test!(
	table_with_view,
