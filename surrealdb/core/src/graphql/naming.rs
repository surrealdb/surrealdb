...
use super::tables::is_valid_graphql_identifier_pub as is_valid_graphql_identifier;
...
pub(crate) fn table_base_name(tb: &TableDefinition) -> &str {
	match tb.graphql_alias.as_deref() {
		Some(alias) if is_valid_graphql_identifier(alias) => alias,
		_ => tb.name.as_str(),
	}
}
...
/// `is_valid_graphql_identifier` guard remains so legacy catalog entries from
...
pub(crate) fn get_field_name(tb: &TableDefinition) -> String {
	let base = table_base_name(tb);
	if tb.graphql_alias.as_deref().is_some_and(is_valid_graphql_identifier) {
...
// ---------------------------------------------------------------------------
// Internal helpers (kept here so the `graphql::naming` module is self-contained).
// ---------------------------------------------------------------------------
...
fn idiom_to_default_name(idiom: &Idiom) -> String {
	...
	// `tables::idiom_to_graphql_name` for the rare multi-part case.
