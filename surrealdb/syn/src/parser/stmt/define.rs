use reblessive::Stk;
use surrealdb_iam::ScramCredential;
use surrealdb_sql::access::AccessDuration;
use surrealdb_sql::access_type::JwtAccessVerify;
use surrealdb_sql::base::Base;
use surrealdb_sql::filter::Filter;
use surrealdb_sql::index::{DiskAnnParams, Distance, HnswParams, VectorType};
use surrealdb_sql::kind::kind_contains_object;
use surrealdb_sql::statements::define::config::api::{ApiConfig, Middleware};
use surrealdb_sql::statements::define::config::defaults::DefaultConfig;
use surrealdb_sql::statements::define::config::graphql::{GraphQLConfig, TableConfig};
use surrealdb_sql::statements::define::config::{ConfigInner, graphql};
...
use crate::parser::mac::{expected, unexpected};
...
impl Parser<'_> {
	...
	pub async fn parse_define_namespace(
		&mut self,
		stk: &mut Stk,
	...
	pub async fn parse_define_param(&mut self, stk: &mut Stk) -> ParseResult<DefineParamStatement> {
		...
		let name = self.next_token_value::<Param>()?.into_strand().into();
		...
		loop {
			match self.peek_kind() {
				...
				t!("FULLTEXT") => {
					...
					res.index = Index::FullText(surrealdb_sql::index::FullTextParams {
	...
	pub fn parse_graphql_config(&mut self) -> ParseResult<GraphQLConfig> {
	...
	pub fn parse_tables(&mut self) -> ParseResult<Vec<surrealdb_sql::TableName>> {
