...
use surrealdb_sql::index::FullTextParams;
...
use surrealdb_sql::{
	...
	Expr, Fetch, Fetchs, Field, Fields, Function, FunctionCall, Group, Groups, Idiom, Index, Kind,
...
fn statements() -> Vec<TopLevelExpr> {
	...
	vec![
		...
		TopLevelExpr::Expr(Expr::Define(Box::new(DefineStatement::Field(Box::new(
		...
		TopLevelExpr::Expr(Expr::Select(Box::new(SelectStatement {
			...
			version: Expr::Literal(Literal::Datetime(expected_datetime)),
			timeout: Expr::Literal(Literal::None),
			tempfiles: false,
			explain: Some(Explain(true)),
		}))),
		...
		TopLevelExpr::Expr(Expr::Literal(Literal::Uuid(uuid::Uuid::from_u128(
