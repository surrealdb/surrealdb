...
use surrealdb_sql::Param;
use surrealdb_sql::language::Language;
use surrealdb_strand::Strand;

use crate::error::bail;
use crate::lexer::Lexer;
use crate::lexer::compound::{self, ParsedInt};
use crate::parser::mac::unexpected;
use crate::parser::{ParseResult, Parser};
use crate::token::{Span, TokenKind, t};
...
impl TokenValue for std::time::Duration {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
...
impl TokenValue for surrealdb_types::Duration {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(Self::from(std::time::Duration::from_token(parser)?))
	}
}

impl TokenValue for chrono::DateTime<chrono::Utc> {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("d\"") | t!("d'") => {
				...
				let datetime = Lexer::lex_datetime(str).map_err(|e| {
					e.update_spans(|span| {
...
impl TokenValue for surrealdb_types::Datetime {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(Self::from(chrono::DateTime::<chrono::Utc>::from_token(parser)?))
	}
}

impl TokenValue for uuid::Uuid {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("u\"") | t!("u'") => {
				...
				let uuid = Lexer::lex_uuid(str).map_err(|e| {
					e.update_spans(|span| {
						let range = span.to_range();
						let start =
							Lexer::escaped_string_offset(parser.span_str(token.span), range.start);
...
impl TokenValue for surrealdb_types::Uuid {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(Self::from(uuid::Uuid::from_token(parser)?))
	}
}
...
impl TokenValue for surrealdb_sql::file::File {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(surrealdb_types::File::from_token(parser)?.into())
	}
}

impl TokenValue for bytes::Bytes {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("b\"") | t!("b'") => {
...
impl TokenValue for surrealdb_types::Bytes {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(Self::from(bytes::Bytes::from_token(parser)?))
	}
}

impl TokenValue for regex::Regex {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let peek = parser.peek();
		match peek.kind {
			t!("/") => {
...
impl TokenValue for surrealdb_types::Regex {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		Ok(Self::from(regex::Regex::from_token(parser)?))
	}
}
...
impl Parser<'_> {
	...
	pub fn parse_ident(&mut self) -> ParseResult<Strand> {
