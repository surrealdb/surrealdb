...
use crate::access_type::{BearerAccess, BearerAccessSubject};
use crate::arbitrary::{
...
use crate::statements::define::{
	DefineAccessStatement, DefineAnalyzerStatement, DefineFieldStatement, DefineUserStatement,
};
use crate::{
	AccessType, Ast, Base, BinaryOperator, Data, DefineIndexStatement, Expr, Index,
	InsertStatement, KillStatement, Kind, Literal, Permission, Permissions, SelectStatement,
	TopLevelExpr, View,
};
...
impl<'a> Arbitrary<'a> for DefineAccessStatement {
	fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
		...
		let duration = crate::access::AccessDuration {
			grant: u.arbitrary()?,
			token: u.arbitrary()?,
			session: u.arbitrary()?,
		};
...
impl<'a> arbitrary::Arbitrary<'a> for DefineFieldStatement {
	fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
		...
		let flexible = if let Some(kind) = &field_kind
			&& crate::kind::kind_contains_object(kind)
		{
			u.arbitrary()?
		} else {
			false
