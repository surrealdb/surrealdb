...
// ---------------------------------------------------------------------------
// Value parsers for clap (only compiled with the `cli` feature)
// ---------------------------------------------------------------------------

#[cfg(feature = "cli")]
fn parse_generation_alloc_limit(s: &str) -> Result<usize, String> {
	let n = s.parse::<u32>().map_err(|e| e.to_string())?;
	Ok(2usize.pow(n.min(28)))
}

#[cfg(feature = "cli")]
fn parse_path_list(s: &str) -> Result<Vec<PathBuf>, String> {
