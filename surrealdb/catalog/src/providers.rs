...
pub type BoxProviderFut<'a, T> = Pin<Box<dyn Future<Output = T> + Send + 'a>>;
...
/// Cooperative cancellation for long-running provider operations.
///
/// Sequence batch allocation retries until it wins a slot; the probe lets the
/// surrounding operation's deadline or cancellation interrupt that loop.
/// `None` disables the check.
pub trait CancellationProbe: Send + Sync {
	/// Errors once the surrounding operation is cancelled or past its
	/// deadline.
	fn expect_not_timedout(&self) -> BoxProviderFut<'_, Result<()>>;
}

impl<T: CancellationProbe> CancellationProbe for std::sync::Arc<T> {
	fn expect_not_timedout(&self) -> BoxProviderFut<'_, Result<()>> {
		T::expect_not_timedout(self)
	}
}

pub trait NodeProvider: Sync {
...
pub trait NamespaceProvider: Sync {
...
pub trait AuthorisationProvider: Sync {
