//! IndexedDB-specific behaviour tests: on-disk format compatibility,
//! cross-instance persistence, the pending-state overlay (same-transaction
//! conditionals, scan merging, shadowed-row budgeting, witness recording),
//! OCC read-set validation granularity, and the rollback-after-release
//! savepoint interleaving the shared suite deliberately leaves
//! backend-divergent. Generic contract behaviour is covered by the
//! `surrealdb-kvs-test` suite run from the `suite` test target.
...
use surrealdb_kvs_test::{TestBackend, TestDs};
...
#[wasm_bindgen_test]
async fn kvs_suite() {
	// Every test gets its own uniquely-named database: IndexedDB state
	// persists per browser profile, so a fixed name would leak state
	// between tests and (via open connections) block deletion.
	static COUNTER: AtomicUsize = AtomicUsize::new(0);
	let backend = TestBackend::new("indxdb", || {
		// The `clear` JS-binding future is not `Send`; the factory future
		// must be. Sound for the same reason as the backend itself: wasm
		// without `atomics` is single-threaded.
		common::future::assert_send(async {
			let name = format!("kvs-suite-{}", COUNTER.fetch_add(1, Ordering::Relaxed));
			clear(&name).await.expect("failed to clear the test database");
			let ds = Datastore::new(&name).await.expect("failed to open the test database");
			TestDs::from_builder(Box::new(ds))
		})
	});
	let ran = surrealdb_kvs_test::run_all(&[backend], |line| console_log!("{line}")).await;
	assert!(ran > 0, "inventory registered no tests under the wasm runner");
}
