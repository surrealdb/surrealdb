...
use surrealdb_catalog::TaskLease;
...
use crate::{Error as KvsError, Transaction, TransactionType, storage_error};

#[derive(Debug, Clone, Copy, Eq, PartialEq, Ord, PartialOrd)]
pub enum TaskLeaseType {
...
impl LeaseHandler {
	...
	pub async fn has_lease(&self) -> Result<bool> {
		...
		while tempo < MAX_BACKOFF {
			...
			match self.check_lease().await {
				Ok(r) => return Ok(r),
				Err(e)
					// Only the bare shape can reach here: the wrapping variant belongs
					// to core's error type, which this layer cannot construct.
					if matches!(
	...
	/// }
	/// ```
	pub async fn try_maintain_lease(&self) -> Result<bool> {
		self.ensure_not_cancelled()?;
		let now = Utc::now();
	...
	async fn acquire_new_lease(&self, current: DateTime<Utc>) -> Result<bool> {
		...
		match res {
			...
			Err(e) => {
				tx.cancel().await?;
				if matches!(storage_error(&e), Some(KvsError::TransactionConditionNotMet)) {
					Ok(false)
				} else {
					Err(e)
				}
...
mod tests {
	...
	use crate::factory::TransactionFactory;
	use crate::sequences::Sequences;
	use crate::tasklease::{LeaseHandler, LeaseStatus, TaskLeaseType};
